 ...
 */
function init(fullUri, options, callback) {
    let url;
    let retries = 0;
    let lastError;
...
function configureDb(callback) {
    ...
    if (currentConfig.deviceRegistry?.type === 'mongodb') {
        ...
        if (mongoCfg?.uri) {
            ...
            init(mongoCfg.uri, options, callback);
            return;
        }

        if (!mongoCfg?.uri) {
            logger.fatal(context, 'MONGODB-003: No URI found for MongoDB driver.');
            callback(new errors.BadConfiguration('No URI found for MongoDB driver'));
            return;
        }
    } else {
        callback();
