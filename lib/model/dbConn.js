...
function init(host, db, port, username, password, options, callback) {
    ...
    function retryCheck() {
        return !defaultDb && retries < (config.getConfig().mongodb && config.getConfig().mongodb.retries ||
    ...
    function connectionAttempt(url, options, callback) {
        ...
        defaultDb = mongoose.createConnection(url, options, callback);
    }

    function tryCreateConnection(callback) {
        var attempt = async.apply(connectionAttempt, url, options, callback),
            seconds = (config.getConfig().mongodb && config.getConfig().mongodb.retry_time ||
