...
async function load() {
    try {
        await Device.collection.dropIndex('service_1_subservice_1_id_1_apikey_1');
    } catch (err) {
        console.log('El �ndice no exist�a o no se pudo eliminar:', err.message);
    }
    ...
    await Device.collection.createIndex(
        ...
        {
            unique: true,
            name: 'unique_service_subservice_id_apikey',
            background: true
        }
    );
