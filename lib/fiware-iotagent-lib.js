...
function registerIoTAgent(newConfig, callback) {
    ...
    logger.debug('Registering IoT Agent in Context Broker at [%s]', options.url);
...
function sendSubscription(deviceId, deviceType, callback) {
    var options = {
            url: 'http://' + config.contextBroker.host + ':' + config.contextBroker.port + '/NGSI10/subscribeContext',
            method: 'POST',
            json: {
                entities: [
                    {
                ...
                reference: config.providerUrl + '/notification',
                duration: 'P1M',
                notifyConditions: [
                    {
                        type: 'ONCHANGE',
    ...
    logger.debug('Sending device context subscription to Context Broker at [%s]', options.url);
    logger.debug('Using the following request:\n\n%s\n\n', JSON.stringify(options, null, 4));
...
function registerDevice(id, type, attributes, callback) {
    async.series([
        ...
        sendRegistrations,
        async.apply(sendSubscription, id, type)
    ], function(error) {
        callback(error);
    });
}
