...
function doActivate(newConfig, callback) {
    ...
    async.series([
        registry.init,
        statsRegistry.init,
        apply(ngsi.init, registry, groupRegistry),
        apply(subscriptions.init, registry),
        apply(iotManager.init, groupRegistry),
        apply(deviceService.init, registry, groupRegistry),
        apply(groupConfig.init, groupRegistry),
