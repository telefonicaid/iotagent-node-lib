...
function globalErrorHandler(err) {
    logger.fatal(
        context,
        'An unexpected exception has been raised. Ignoring: %s',
        err
    );
}
...
function doActivate(newConfig, callback) {
    var registry, groupRegistry, commandRegistry, securityService;

    logger.format = logger.formatters.pipe;

    if (newConfig.contextBroker) {
        if (
            !newConfig.contextBroker.url &&
            newConfig.contextBroker.host &&
            newConfig.contextBroker.port
        ) {
            newConfig.contextBroker.url =
                'http://' +
                newConfig.contextBroker.host +
            ...
            newConfig.contextBroker.url =
                'http://' + newConfig.contextBroker.host;
        }
    }

    if (newConfig.iotManager) {
        if (
            !newConfig.iotManager.url &&
            newConfig.iotManager.host &&
            newConfig.iotManager.port
        ) {
            newConfig.iotManager.url =
                'http://' +
                newConfig.iotManager.host +
    ...
    if (newConfig.authentication && newConfig.authentication.enabled) {
        ...
        if (
            ...
            newConfig.authentication.url =
                'http://' + newConfig.authentication.host;
        }
    }
    ...
    if (
        newConfig.deviceRegistry &&
        newConfig.deviceRegistry.type &&
        newConfig.deviceRegistry.type === 'mongodb'
    ) {
        logger.info(
            context,
            'MongoDB Device registry selected for NGSI Library'
        );
        ...
        logger.info(
            context,
            'Falling back to Transient Memory registry for NGSI Library'
        );
...
exports.mergeDeviceWithConfiguration =
...
exports.setRemoveConfigurationHandler =
    contextServer.setRemoveConfigurationHandler;
...
exports.addDeviceProvisionMiddleware =
    contextServer.addDeviceProvisionMiddleware;
exports.addNotificationMiddleware = contextServer.addNotificationMiddleware;
exports.addConfigurationProvisionMiddleware =
    contextServer.addConfigurationProvisionMiddleware;
