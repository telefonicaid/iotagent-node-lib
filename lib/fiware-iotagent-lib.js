...
function activateStatLogs(newConfig, callback) {
    // global stats are always initialized, since they
    // will be collected regardless of `stats.interval`
    logActions = [
    ...
    // However, local stats will only be periodically
    // logged / cleared if `stats.interval` is set
    if (newConfig.stats && newConfig.stats.interval) {
        logActions.push(apply(statsRegistry.addTimerAction, statsRegistry.logStats));
    }
    async.series(logActions, callback);
}
