...
module.exports = {
  RegistrationError: function(id, type) {
    ...
    this.message =
      'Error registering context provider for device: ' +
      id +
      ' of type: ' +
      type;
  },
  UnregistrationError: function(id, type) {
    ...
    this.message =
      'Error unregistering context provider for device: ' +
      id +
      ' of type: ' +
      type;
  },
  EntityGenericError: function(id, type, details, code) {
    ...
    this.message =
      'Error accesing entity data for device: ' + id + ' of type: ' + type;
  ...
  EntityNotFound: function(id) {
    ...
    this.message =
      'The entity with the requested id [' + id + '] was not found.';
    this.code = 404;
  },
  ...
  DuplicateDeviceId: function(id) {
    ...
    this.message =
      'A device with the same pair (Service, DeviceId) was found:' + id;
    this.code = 409;
  },
  ...
  SecurityInformationMissing: function(type) {
    ...
    this.message =
      'Some security information was missing for device type:' + type;
  },
  TokenRetrievalError: function(trust, msg) {
    this.name = 'TOKEN_RETRIEVAL_ERROR';
    this.message =
      'An error occurred trying to retrieve a token with trust [' +
      trust +
      ']: ' +
      msg;
  },
  ...
  AuthenticationError: function(trust) {
    ...
    this.message =
      'The trust configured for the device was rejected: [' + trust + ']';
  },
  BadConfiguration: function(msg) {
    this.name = 'BAD_CONFIGURATION';
    this.message =
      'The application startup failed due to a bad configuration:' + msg;
  },
  ...
  MismatchedService: function(service, subservice) {
    ...
    this.message =
      "The declared service didn't match the stored one in the entity";
    this.code = 403;
  },
  ...
  CommandNotFound: function(name) {
    ...
    /*jshint quotmark: double */
    this.message =
      "Couldn't update the command because no command with the name [" + name;
    /*jshint quotmark: single */
    this.message = this.message + '] was found.';
    this.code = 400;
  },
  MissingConfigParams: function(missing) {
    this.name = 'MISSING_CONFIG_PARAMS';
    this.message =
      'The following mandatory configuration parameters were missing: ' +
