...
function mergeAttributes(attrList1, attrList2, typeInformation) {
    const finalCollection = _.clone(attrList1);
    const additionalItems = [];
    let found;
    const isNGSILD =
        config.checkNgsiLD(typeInformation) || config.checkNgsiLD(config.getConfig().contextBroker.ngsiVersion);
    ...
    for (let i = 0; i < attrList2.length; i++) {
        ...
        if (!found) {
            if (isNGSILD) {
                ...
                if (!(Number.isNaN(attrList2[i].value) || attrList2[i].value == null)) {
...
function update(entity, typeInformation, callback) {
    ...
    function processEntityUpdateNgsi2(attributes) {
        ...
        attributes = mergeAttributes(attributes, expressionAttributes, typeInformation);
        return attributes;
    }
