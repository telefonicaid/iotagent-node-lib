...
function extractContext(attributeList) {
    ...
    for (let i = 0; i < attributeList.length; i++) {
        if (isNaN(attributeList[i].value)) {
            ...
            const floatValue = Number.parseFloat(attributeList[i].value);
...
function checkTransformationMap(tranformsMap) {
    ...
    if (typeof tranformsMap !== 'object') {
        ...
        const wrongList = [];
        for (const transformation in tranformsMap) {
...
function setTransforms(configMap) {
    ...
    const [error, message, cleanTranformsMap] = checkTransformationMap(tranformsMap);
    if (!error) {
