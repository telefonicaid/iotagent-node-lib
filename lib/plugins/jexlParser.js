...
jexl.addTransform('length', (arr) => arr.length);
...
jexl.addTransform('replaceallregexp', (str, reg, to) => str.replaceAll(new RegExp(reg), to));
...
jexl.addTransform(
    'thmapper',
    (val, values, choices) =>
        choices[values.reduce((acc, curr, i, arr) => (acc ? acc : val <= curr ? (acc = i) : (acc = null)), null)]
);
