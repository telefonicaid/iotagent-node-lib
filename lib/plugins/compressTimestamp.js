...
function processAttributeUpdate(attribute) {
    if (attribute.type && attribute.type.toLowerCase && attribute.type.toLowerCase() === 'timestamp') {
        attribute.value = fromBasicToExtended(attribute.value);
    }

    return attribute;
}

function processAttributeQuery(attribute) {
    if (attribute.type && attribute.type.toLowerCase && attribute.type.toLowerCase() === 'timestamp') {
        attribute.value = fromExtendedToBasic(attribute.value);
    }
...
function processEntityUpdate(entity) {
    entity.attributes = entity.attributes.map(processAttributeUpdate);
    return entity;
}

function processEntityQuery(entity) {
    entity.contextElement.attributes = entity.contextElement.attributes.map(processAttributeQuery);
    return entity;
}
