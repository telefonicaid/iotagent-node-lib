 ...
 *
 * @param {Array} attributes        Array of original attributes coming from the single-entity device.
 ...
 */
function generateNewCEs(entity, newEntities, entityTypes, multiEntityAttributes) {
    var result = [],
        newEntityAttributes,
        newEntityAttributeNames;

    for (var i = 0; i < newEntities.length; i++) {
        newEntityAttributeNames = _.pluck(multiEntityAttributes.filter(function (item) {
            return item.entity_name === newEntities[i];
        }), 'name');

        newEntityAttributes = entity.contextElements[0].attributes.filter(function(item) {
...
function updateAttribute(entity, typeInformation, callback) {
    ...
    entity.contextElements = entity.contextElements.concat(
        generateNewCEs(entity, newEntities, entityTypes, multiEntityAttributes));
