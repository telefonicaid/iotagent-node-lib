...
function processEnvironmentVariables() {
    const environmentVariables = [
        ...
        'IOTA_REDIS_ENABLED',
        'IOTA_REDIS_DEVICE_HOST',
        'IOTA_REDIS_DEVICE_PORT',
        'IOTA_REDIS_DEVICE_PASSWORD',
        'IOTA_REDIS_DEVICE_DB',
        'IOTA_REDIS_DEVICE_TTL',
        'IOTA_REDIS_GROUP_HOST',
        'IOTA_REDIS_GROUP_PORT',
        'IOTA_REDIS_GROUP_PASSWORD',
        'IOTA_REDIS_GROUP_DB',
    ...
    if (process.env.IOTA_REDIS_ENABLED) {
        config.redis.enabled = process.env.IOTA_REDIS_ENABLED === 'true';
    }
    const redis = config.redis;
    if (redis.enabled) {
        redis.deviceHost = redis.deviceHost || 'localhost';
        redis.devicePort = redis.devicePort || 6379;
        redis.deviceDB = redis.deviceDB || 1;
        redis.deviceTTL = redis.deviceTTL || 600;
        redis.groupHost = redis.groupHost || 'localhost';
        redis.groupPort = redis.groupPort || 6379;
        redis.groupDB = redis.groupDB || 0;
        redis.groupTTL = redis.groupTTL || 600;

        if (process.env.IOTA_REDIS_DEVICE_HOST) {
            config.redis.deviceHost = process.env.IOTA_REDIS_DEVICE_HOST;
        }
        if (process.env.IOTA_REDIS_DEVICE_PORT) {
            config.redis.devicePort = process.env.IOTA_REDIS_DEVICE_PORT;
        }
        if (process.env.IOTA_REDIS_DEVICE_PASSWORD) {
            config.redis.devicePassword = process.env.IOTA_REDIS_DEVICE_PASSWORD;
        }
        if (process.env.IOTA_REDIS_DEVICE_DB) {
            config.redis.deviceDB = process.env.IOTA_REDIS_DEVICE_DB;
        }
        if (process.env.IOTA_REDIS_DEVICE_TTL) {
            config.redis.deviceTTL = process.env.IOTA_REDIS_DEVICE_TTL;
        }

        if (process.env.IOTA_REDIS_GROUP_HOST) {
            config.redis.deviceHost = process.env.IOTA_REDIS_GROUP_HOST;
        }
        if (process.env.IOTA_REDIS_GROUP_PORT) {
            config.redis.devicePort = process.env.IOTA_REDIS_GROUP_PORT;
        }
        if (process.env.IOTA_REDIS_GROUP_PASSWORD) {
            config.redis.devicePassword = process.env.IOTA_REDIS_GROUP_PASSWORD;
        }
        if (process.env.IOTA_REDIS_GROUP_DB) {
            config.redis.deviceDB = process.env.IOTA_REDIS_GROUP_DB;
        }
        if (process.env.IOTA_REDIS_GROUP_TTL) {
            config.redis.groupTTL = process.env.IOTA_REDIS_GROUP_TTL;
        }
    }
