...
function processEnvironmentVariables() {
    ...
    // Stats Configuration
    if (anyIsSet(statsVariables)) {
        config.stats = config.stats || {};
    }
    
    if (process.env.IOTA_STATS_INTERVAL) {
        const interval = parseInt(process.env.IOTA_STATS_INTERVAL);
        if (!isNaN(interval) && interval > 0) {
            config.stats.interval = interval;
        }
    }

    if (process.env.IOTA_STATS_PERSISTENCE) {
        config.stats.persistence = process.env.IOTA_STATS_PERSISTENCE === 'true'
    }
}
