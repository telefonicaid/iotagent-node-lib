...
function processEnvironmentVariables() {
   ...
   if (Array.isArray(config.contextBroker.jsonLdContext) && config.contextBroker.jsonLdContext.length === 1){
    ...
    config.server.ldPayloadSupport = config.server.ldPayloadSupport || {supportNull: true, supportDatasetId: true};

    if (process.env.IOTA_LD_SUPPORT_NULL) {
        config.server.ldPayloadSupport.supportNull = process.env.IOTA_LD_SUPPORT_NULL === 'true';
    }
    if (process.env.IOTA_LD_SUPPORT_DATASET_ID) {
        config.server.ldPayloadSupport.supportDatasetId = process.env.IOTA_LD_SUPPORT_DATASET_ID === 'true';
    }
