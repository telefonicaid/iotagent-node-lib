   ...
   if (Array.isArray(config.contextBroker.jsonLdContext) && config.contextBroker.jsonLdContext.length === 1){
    ...
    if (redis.enabled) {
        ...
        if (process.env.IOTA_REDIS_GROUP_HOST) {
            config.redis.groupHost = process.env.IOTA_REDIS_GROUP_HOST;
        }
        if (process.env.IOTA_REDIS_GROUP_PORT) {
            config.redis.groupPort = process.env.IOTA_REDIS_GROUP_PORT;
        }
        if (process.env.IOTA_REDIS_GROUP_PASSWORD) {
            config.redis.groupPassword = process.env.IOTA_REDIS_GROUP_PASSWORD;
        }
        if (process.env.IOTA_REDIS_GROUP_DB) {
            config.redis.groupDB = process.env.IOTA_REDIS_GROUP_DB;
        }
