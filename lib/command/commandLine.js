...
function addGroup(commands) {
    clUtils.getWriter().log('Adding device groups to host [%s:%s] from file [%s]', configIot.host, configIot.port, commands[0]);
...
function listGroups(commands) {
    ...
    request(options, function(error, result, body) {
        if (error) {
            ...
            clUtils.getWriter().log(JSON.stringify(result.headers));
            if (error) {
                clUtils.getWriter().log('Couldn\'t connect with the provisioning server: ' + error.toString());
            } else if (result.statusCode === 200 && body) {
                var parsedBody = JSON.parse(body);
