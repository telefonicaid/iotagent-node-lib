    ...
    function translateItem(item, innerCb) {
        ...
        for (var i in item) {
            if (item.hasOwnProperty(i) && translationKey[i]) {
                ...
                logger.warn('Attribute [%s] was not found for item translation', i);
            }
        }
...
function migrate(dbConfig, originDb, targetDb, service, subservice, callback) {
    ...
    function getCollection(db, name, innerCb) {
        db.collection(name).find({}).toArray(innerCb);
    }
