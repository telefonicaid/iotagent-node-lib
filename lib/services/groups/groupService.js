...
function createGroup(groupSet, callback) {
    ...
    if (groupSet.hasOwnProperty(constants.CONFIGGROUP_TERM)) {
        logger.debug(
            context,
            'Creating new set of %d %s',
            groupSet[constants.CONFIGGROUP_TERM].length,
            constants.CONFIGGROUP_TERM
        );

        for (let i = 0; i < groupSet[constants.CONFIGGROUP_TERM].length; i++) {
            insertions.push(async.apply(validateGroup, groupSet[constants.CONFIGGROUP_TERM][i]));
            insertions.push(async.apply(config.getGroupRegistry().create, groupSet[constants.CONFIGGROUP_TERM][i]));
            insertedGroups.push(groupSet[constants.CONFIGGROUP_TERM][i]);
        }
    } else {
...
function getEffectiveApiKey(service, subservice, type, callback) {
    ...
    find(constants.CONFIGGROUP_TERM, service, subservice, type, handleFindGroup);
}
