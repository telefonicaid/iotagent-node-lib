...
function validateGroup(group, callback) {
    ...
    function checkServiceAndSubservice(innerCb) {
        config.getGroupRegistry().findConfigGroups(group.service, group.subservice, generateDuplicateHandler(innerCb));
    }
...
function createGroup(groupSet, callback) {
    ...
    logger.debug(context, 'Creating new set of %d services', groupSet.configGroups.length);

    for (let i = 0; i < groupSet.configGroups.length; i++) {
        insertions.push(async.apply(validateGroup, groupSet.configGroups[i]));
...
function getEffectiveApiKey(service, subservice, type, callback) {
    ...
    findConfigGroups(service, subservice, type, handleFindGroup);
}
