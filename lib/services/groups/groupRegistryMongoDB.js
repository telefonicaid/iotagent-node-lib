...
function find(service, subservice, callback) {
    ...
    logger.debug(context, 'Looking for device group with service [%s] and subservice [%s].', service, subservice);

    query = Group.model.find({service: service, subservice: subservice});
    query.select({__v: 0});
    ...
    query.exec(function handleGet(error, data) {
        if (error) {
            ...
            callback(null, data.map(toObjectFn));
        } else {
            logger.debug(context, 'Device group for service [%j] and subservice [%j  not found', service, subservice);
