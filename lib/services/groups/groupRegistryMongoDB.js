...
const memoryCache =  
    config.getConfig().deviceRegistry.cache.enabled ?
    cacheManager.caching({
        store: 'memory',
        max: config.getConfig().deviceRegistry.cache.groupSize, 
        ttl: config.getConfig().deviceRegistry.cache.groupTTL
    }) : undefined;
let context = {
...
function listGroups(service, limit, offset, callback) {
    ...
    async.series([
        ...
        Group.model.countDocuments.bind(Group.model, condition)
...
function findBy(fields) {
    return function() {
        ...
        if (memoryCache) {
            memoryCache.wrap(JSON.stringify(queryObj), cacheCallback => {
