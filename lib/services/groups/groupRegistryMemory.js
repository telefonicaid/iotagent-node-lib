...
function listGroups(endpoint, service, limit, offset, callback) {
    ...
    if (endpoint != null && endpoint.toLowerCase().includes(constants.CONFIGGROUP_TERM.toLowerCase())) {
        const configGroupsObject = {};
        configGroupsObject.count = filteredGroups.length;
        configGroupsObject[constants.CONFIGGROUP_TERM] = result;
        callback(null, configGroupsObject);
    } else {
        callback(null, {
...
function find(endpoint, service, subservice, callback) {
    ...
    if (result.length > 0) {
        ...
        if (endpoint.toLowerCase().includes(constants.CONFIGGROUP_TERM.toLowerCase())) {
            const configGroupsObject = {};
            configGroupsObject.count = result.length;
            configGroupsObject[constants.CONFIGGROUP_TERM] = result;
            return callback(null, configGroupsObject);
        } else {
