...
function addCommand(service, subservice, device, command, callback) {
    logger.debug(context, 'Adding command [%s] to the queue for device [%s]', command.name, device.id);
    if (command.expression) {
        ...
        let attrList = [
            ...
            { name: 'subservice', type: 'String', value: subservice }
        ];
        ...
        try {
            cmdValueRes = parser.applyExpression(command.expression, ctxt, device);
        } catch (e) {
            // nothing to do
        }
        command.value = cmdValueRes ? cmdValueRes : command.expression;
    }
