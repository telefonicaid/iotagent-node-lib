...
function findCommand(service, subservice, deviceId, name, callback) {
    ...
    logger.debug('Looking for command [%s] for device [%s]', name, deviceId);
    ...
    query.exec(function handleGet(error, data) {
        if (error) {
            logger.debug('Internal MongoDB Error getting command: %s', error);

            callback(new errors.InternalDbError(error));
        } else if (data) {
            callback(null, data);
        } else {
            logger.debug('Command for DeviceID [%j] with name [%j] not found', deviceId, name);
...
function remove(service, subservice, deviceId, name, callback) {
    logger.debug('Removing command for service [%s] subservice [%s] and DeviceID [%s] with name [%s]',
        ...
        if (error) {
            ...
            Command.model.remove({ _id: command._id }, function(error, commandResult) {
                if (error) {
                    ...
                    logger.debug('Command [%s] successfully removed.', name);

                    callback(null, commandResult);
                } else {
                    logger.debug('Entity [%s] not found for removal.', name);
...
function removeFromDate(creationDate, callback) {
    Command.model.remove({ creationDate: { $lt: creationDate } }, function(error, commandResult) {
        if (error) {
            logger.debug('Internal MongoDB Error removing expired commands: %s', error);

            callback(new errors.InternalDbError(error));
        } else if (commandResult && commandResult.result && commandResult.result.n > 0) {
            logger.debug('Expired commands successfully removed from MongoDB for date [%s]', creationDate);

            callback(null, commandResult);
        } else {
            logger.debug('Expired commands not found for removal for date [%s]', creationDate);
