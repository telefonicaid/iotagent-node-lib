...
function listCommands(service, subservice, deviceId, callback) {
    ...
    async.series([query.exec.bind(query), Command.model.count.bind(Command.model, condition)], function(
        error,
        results
    ) {
        callback(error, {
    ...
    if (Command.model) {
        Command.model.find(query).exec(function(error, commandList) {
            if (!error && commandList && commandList.length > 0) {
                Command.model.remove(query, function(error) {
                    if (error) {
