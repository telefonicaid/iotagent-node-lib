...
function removeFromDate(creationDate) {
    Command.model.remove({ creationDate: { $lt: creationDate } }, function(error, commandResult) {
        if (error) {
            ...
            logger.debug('Expired commands [%s] successfully removed from MongoDB.', name);

            callback(null, commandResult);
        } else {
            logger.debug('Commands [%s] not found for removal.', name);
