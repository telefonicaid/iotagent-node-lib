...
function auth(trust, callback) {
    ...
    logger.debug(context, 'Authentication on the OAuth2 provider [%s] using trust [%s]', url, trust);
    ...
    request(options, function handleResponse(error, response, body) {
        ...
        } else if (response.statusCode === 401 || (response.statusCode === 400 && JSON.parse(response.body)['error_description'] == "Invalid refresh token")) {
