...
function raise(alarmName, description) {
    if (!alarmRepository[alarmName]) {
        ...
        // NOTE: statsRegistry.add is actually synchronous (does not return a promise),
        // we can call it synchronously.
...
function intercept(alarmName, targetFn) {
    function interceptCallback(callback) {
        return function () {
            ...
            const error = originalArguments.slice(0, 1);
    ...
    return function () {
        /* eslint-disable-next-line prefer-rest-params */
        const originalArguments = Array.prototype.slice.call(arguments);
        const callback = originalArguments.splice(-1);
