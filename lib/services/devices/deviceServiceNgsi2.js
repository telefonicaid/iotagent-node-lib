...
var request = require('request'),
    ...
    subscriptions = require('../ngsi2/subscriptionService'),
...
/**
 * Creates the initial entity representing the device in the Context Broker.
 *
 * @param {Object} deviceData       Object containing all the deviceData needed to send the registration.
 * @param {Object} newDevice        Device object that will be stored in the database.
 */
function createInitialEntity(deviceData, newDevice, callback) {
    ...
    function formatAttributes(originalVector) {
        ...
        if (originalVector && originalVector.length) {
            for (var i = 0; i < originalVector.length; i++) {
                attributesJson[originalVector[i].name] = {
    ...
    function formatStaticAttributes(originalVector) {
        var attributesJson = {};
    ...
    function formatCommands(originalVector) {
        ...
        if (originalVector && originalVector.length) {
            for (var i = 0; i < originalVector.length; i++) {
                attributesJson[originalVector[i].name + constants.COMMAND_STATUS_SUFIX] = {
                    type: constants.COMMAND_STATUS,
                    value: 'UNKNOWN'
                };

                attributesJson[originalVector[i].name + constants.COMMAND_RESULT_SUFIX] = {
    ...
    _.extend(options.json, formatAttributes(deviceData.active));
    _.extend(options.json, formatStaticAttributes(deviceData.staticAttributes));
    _.extend(options.json, formatCommands(deviceData.commands));
