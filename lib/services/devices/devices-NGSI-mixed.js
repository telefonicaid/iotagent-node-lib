 ...
 */
function updateRegisterDeviceNgsiMixed(deviceObj, typeNameUpdated, callback) {
    if (config.checkNgsiLD(deviceObj)) {
        deviceHandlerLD.updateRegisterDevice(deviceObj, typeNameUpdated, callback);
    } else {
        deviceHandlerV2.updateRegisterDevice(deviceObj, typeNameUpdated, callback);
    }
}
