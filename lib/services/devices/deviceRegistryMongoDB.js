...
const memoryCache = config.getConfig().deviceRegistry.cache.enabled ?
    cacheManager.caching({
        store: 'memory', 
        max: config.getConfig().deviceRegistry.cache.deviceSize, 
        ttl: config.getConfig().deviceRegistry.cache.deviceTTL
    }) : undefined;
let context = {
...
function getDeviceById(id, service, subservice, callback) {
    ...
    if (memoryCache) {
        memoryCache.wrap(JSON.stringify(queryParams), cacheCallback  => {
