...
function init(callback) {
    if (!config.getConfig().deviceRegistry.host) {
        logger.fatal('No host found for MongoDB driver.');
        callback(new errors.BadConfiguration('No host found for MongoDB driver'));
    } else {
        var dbName = config.getConfig().deviceRegistry.db,
            port = config.getConfig().deviceRegistry.port || 27017;

        if (!config.getConfig().deviceRegistry.db) {
            dbName = dbService.DEFAULT_DB_NAME;
        }

        dbService.init(config.getConfig().deviceRegistry.host, dbName, port, {}, callback);
    }
}
