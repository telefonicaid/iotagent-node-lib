...
/**
 * Sets up the in-memory and Redis caches for devices, should one be required.
 */
function initialiseCaches(config) {
    function isCacheableValue(value) {
        if (value !== null && value !== false && value !== undefined) {
            return !value.dontCache;
        }
        return false;
    }
    function initialiseRedis(redis, isCacheableValue) {
        const redisCache = cacheManager.caching({
            store: redisStore,
            host: redis.deviceRedisHost,
            port: redis.deviceRedisPort,
            password: redis.deviceRedisPassword,
            db: redis.deviceRedisDB,
            ttl: redis.deviceRedisTTL,
            isCacheableValue
        });
        // listen for redis connection error event
        redisClient = redisCache.store.getClient();
        redisClient.on('error', (error) => {
            logger.error(context, 'Redis for Devices: ' + error);
        });
        redisClient.on('connect', () => {
            logger.info(context, 'Successfully connected to Redis for Devices');
        });
        return redisCache;
    }
...
function update(device, callback) {
    ...
    getDeviceById(device.id, device.service, device.subservice, function (error, data) {
        if (error) {
            ...
            data.dontCache = device.dontCache;
