...
function storeDevice(newDevice, callback) {
    ...
    if (registeredDevices[newDevice.service][newDevice.id]) {
        ...
        registeredDevices[newDevice.service][newDevice.id] = newDevice;
...
function removeDevice(id, service, subservice, callback) {
    var services = Object.keys(registeredDevices);

    if (!subservice && !callback) {
        callback = service;
    }
...
function getByName(name, service, subservice, callback) {
    var devices = _.values(registeredDevices[service]),
        device;;
...
function update(device, callback) {
    registeredDevices[device.service][device.id] = device;
    callback(null, device);
}
