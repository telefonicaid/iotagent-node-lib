...
function sendSubscriptionsNgsi2(unregister, deviceData, callback) {
    ...
    // TBD Create trigger and content of subscription using attrs and deviceData
    const trigger = {};
    const content = {};

    if (attrs.length === 0) {
        logger.debug(context, 'Subscription is not needed. Device without lazy atts or commands');
        return callback(null, deviceData);
    }

    logger.debug(context, 'Sending v2 device subscriptions to Context Broker at [%s]', options.url);
    logger.debug(context, 'Using the following request:\n\n%s\n\n', JSON.stringify(options, null, 4));
    subscriptionService.subscribeNgsi2(deviceData, triggers, content, callback);
}
