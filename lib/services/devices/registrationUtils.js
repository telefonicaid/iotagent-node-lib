...
function sendRegistrationsNgsi2(unregister, deviceData, callback) {
    ...
    // Check if registration is about lazy
    // Assumming that if lazy attrs there is not commands
    if (
        deviceData.lazy &&
        deviceData.lazy.length > 0 &&
        (!deviceData.commands || (deviceData.commands && deviceData.commands.length === 0))
    ) {
        options.json.provider.supportedForwardingMode = 'query';
    }
    // Check if registration is about command
    // Assumming that if commands  there is not lazy atts
    if (
        deviceData.commands &&
        deviceData.commands.length > 0 &&
        (!deviceData.lazy || (deviceData.lazy && deviceData.lazy.length === 0))
    ) {
        options.json.provider.supportedForwardingMode = 'update';
    }
    ...
    // Check if lazy and commands are present -> 2 registrations should be done
    if (deviceData.lazy && deviceData.lazy.length > 0 && deviceData.commands && deviceData.commands.length > 0) {
        logger.debug(context, 'Sending v2 device registrations to Context Broker at [%s]', options.url);
        options.json.dataProvided.attrs = options.json.dataProvided.attrs
            .concat(formatAttributes(deviceData.lazy))
            .reduce(mergeWithSameName, []);
        // lazy
        var handler = createRegistrationHandlerNgsi2(unregister, deviceData, callback);
        options.json.provider.supportedForwardingMode = 'query';
        logger.debug(context, 'Using the following request:\n\n%s\n\n', JSON.stringify(options, null, 4));
        utils.executeWithSecurity(options, deviceData, handler);
        // commands
        (unregister = false), (handler = createRegistrationHandlerNgsi2(unregister, deviceData, callback));
        options.json.provider.supportedForwardingMode = 'update';
        logger.debug(context, 'Using the following request:\n\n%s\n\n', JSON.stringify(options, null, 4));
        utils.executeWithSecurity(options, deviceData, handler);
    } else {
        // Single registration
        logger.debug(context, 'Sending v2 device registration to Context Broker at [%s]', options.url);
        logger.debug(context, 'Using the following request:\n\n%s\n\n', JSON.stringify(options, null, 4));
