 ...
 * @param {Object} deviceData       Object containing all the deviceData needed to send the registration.
 */
function sendRegistrationsNgsi1(unregister, deviceData, token, callback) {
    var options = {
...
function sendUnregistrationsNgsi2(deviceData, callback) {
    ...
    if (deviceData.registrationId) {
        ...
        request(options, createRegistrationHandlerNgsi2(true, deviceData, callback));        
    } else {
        logger.debug(context, 'No Context Provider registrations found for unregister');
        callback(null, deviceData);
    }
    ...
    if (unregister) {
        ...
        if (deviceData.registrationId) {
            ...
            if (options.json.dataProvided.attrs.length === 0) {
                ...
                request(options, createRegistrationHandlerNgsi2(unregister, deviceData, callback));
            }
        }
    }
}
...
function sendRegistrations(unregister, deviceData, callback) {
    if (config.checkNgsi2()) {
        ...
        executeWithOAuth2Token(sendRegistrationsNgsi1, unregister, deviceData, callback);
    }
}

function executeWithOAuth2Token(operationFunction, unregister, deviceData, callback) {
            ...
            if (config.getConfig().authentication && config.getConfig().authentication.enabled) {
                ...
                if (typeInformation && typeInformation.trust) {
                    async.waterfall([
                        ...
                        apply(operationFunction, unregister, deviceData)
                    ], callback);
                } else {
                    callback(new errors.SecurityInformationMissing(typeInformation.type));
                }
            } else {
                operationFunction(unregister, deviceData, null, callback);
            }
        });        
}
