...
function sendRegistrations(unregister, deviceData, callback) {
    ...
    if (options.json.contextRegistrations[0].attributes.length === 0) {
        ...
        if (config.getConfig().authentication && config.getConfig().authentication.enabled) {
            config.getGroupRegistry().find(deviceData.service, deviceData.subservice, function(error, deviceGroup) {
              ...
              if (typeInformation && typeInformation.trust) {
                  async.waterfall([
                      ...
                      apply(registrationRequest, unregister, deviceData)
                  ], callback);
              } else {
            ...
            registrationRequest(unregister, deviceData, options, callback);
        }
    }
}

function registrationRequest(unregister, deviceData, options, callback){
    logger.debug(context, 'Using the following request:\n\n%s\n\n', JSON.stringify(options, null, 4));
