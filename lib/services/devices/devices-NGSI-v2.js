...
function createInitialEntityNgsi2(deviceData, newDevice, callback) {
    logger.debug(context, 'createInitialEntityNgsiv2 called with deviceData: %j', deviceData);
    if (
        (!newDevice.cmdMode && (!config.getConfig().cmdMode || config.getConfig().cmdMode === 'legacy')) ||
        (newDevice && newDevice.cmdMode === 'legacy')
    ) {
        logger.debug(context, 'Old cmdMode with newDevice: %j', newDevice);
        callback(null, newDevice);
    } else {
        ...
        if (newDevice.cbHost && newDevice.cbHost.indexOf('://') !== -1) {
            options.url = newDevice.cbHost + '/v2/entities?options=upsert';
        } else if (newDevice.cbHost && newDevice.cbHost.indexOf('://') === -1) {
            options.url = 'http://' + newDevice.cbHost + '/v2/entities?options=upsert';
        }
        jsonConcat(options.json, formatCommandsBySubsNgsi2(newDevice.commands));
        logger.debug(context, 'Creating initial entity with newDevice: %j', newDevice);
...
function updateRegisterDeviceNgsi2(deviceObj, previousDevice, entityInfoUpdated, callback) {
    ...
    function extractDeviceDifference(newDevice, oldDevice, callback) {
        ...
        logger.debug(context, 'extractDeviceDifference newDevice %j oldDevice %j difference %j', deviceData);
