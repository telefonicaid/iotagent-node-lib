...
function updateEntityNgsi2(deviceData, updatedDevice, callback) {
    const options = {
        url: config.getConfig().contextBroker.url + '/v2/entities/' + String(deviceData.name) + '/attrs?options=upsert',
        method: 'POST',
        json: {},
        headers: {
            'fiware-service': deviceData.service,
    ...
    if (deviceData.cbHost && deviceData.cbHost.indexOf('://') !== -1) {
        options.url = deviceData.cbHost + '/v2/entities/' + String(deviceData.name) + '/attrs?options=upsert';
    } else if (deviceData.cbHost && deviceData.cbHost.indexOf('://') === -1) {
        options.url = 'http://' + deviceData.cbHost + '/v2/entities/' + String(deviceData.name) + '/attrs?options=upsert';
    }
