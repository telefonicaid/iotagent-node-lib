...
function registerDevice(deviceObj, callback) {
    ...
    function completeSubscriptions(error, deviceData) {
        ...
        logger.debug(context, 'CompleteSubscriptions for device:\n%s', JSON.stringify(deviceData, null, 4));
        ...
        if (deviceData.commands && deviceData.commands.length > 0) {
            ...
            subscriptions.subscribe(deviceData, commandNames, null, function (error) {
                if (error) {
                    logger.debug(context, 'CompleteSubscriptions for device error: %j', error);
                }
                completeRegistrations(error, deviceData);
            });
        } else {
            completeRegistrations(error, deviceData);
        }
