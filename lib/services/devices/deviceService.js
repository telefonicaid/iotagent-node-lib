 ...
 * @param {Object} newDevice        Device object that will be stored in the database.
 */
function createInitialEntityNgsi1(deviceData, newDevice, token, callback) {
    var options = {
...
function createInitialEntity(deviceData, newDevice, callback) {
    if (config.checkNgsi2()) {
        ...
        executeWithOAuth2Token(createInitialEntityNgsi1, deviceData, newDevice, callback);
    }
}
...
function mergeDeviceWithConfiguration(fields, defaults, deviceData, configuration, callback) {
    for (var i = 0; i < fields.length; i++) {
        ...
        if (configuration.trust) {
            deviceData.trust = configuration.trust;
        }
    }
...
function executeWithOAuth2Token(operationFunction,deviceData, newDevice, callback) {
        logger.debug(context, 'executeWithOAuth2Token');
        config.getGroupRegistry().getType(newDevice.type, function(error, deviceGroup) {
            var typeInformation;
            if (error) {
            ...
            if (deviceGroup) {
                ...
                typeInformation = config.getConfig().types[newDevice.type];
            }
            ...
            if (config.getConfig().authentication && config.getConfig().authentication.enabled) {
                ...
                if (typeInformation && typeInformation.trust) {
                    async.waterfall([
                        ...
                        apply(operationFunction, deviceData, newDevice)
                    ], callback);
                } else {
                    callback(new errors.SecurityInformationMissing(typeInformation.type));
                }
            } else {
                operationFunction(deviceData, newDevice, null, callback);
            }
        });        
}
