...
function registerDevice(deviceObj, callback) {
    ...
    function completeSubscriptions(error, deviceData) {
        if (error) {
            return callback(error);
        }

        // Check if there are comands into deviceData
        if (config.ngsiVersion() !== 'ld' && deviceData.commands && deviceData.commands.length > 0) {
            logger.debug(context, 'CompleteSubscriptions searching commands of device: %j', deviceData);
            var commandNames = [];
            for (let cmd of deviceData.commands) {
                // TBD: Filter just new commands
                //if (cmd.commandType) {
            ...
            if (commandNames.length > 0) {
                logger.debug(
                    context,
                    'CompleteSubscriptions subscribe for commands %j of device %j',
                    commandNames,
                    deviceData
                );
                subscriptions.subscribe(deviceData, commandNames, null, callback);
            }
        }
        callback(null);
    }
        ...
        async.waterfall(
            ...
            function (error, results) {
                if (error) {
                    ...
                    config.getRegistry().store(deviceObj, function () {
                        completeSubscriptions(error, deviceData);
                    });
                }
            }
        );
