...
function createInitialEntityNgsi2(deviceData, newDevice, callback) {
    var options = {
        url: config.getConfig().contextBroker.url + '/v2/entities',
        method: 'POST',
        json: {
    ...
    jsonConcat(options.json, formatAttributes(deviceData.active, false));
    jsonConcat(options.json, formatAttributes(deviceData.staticAttributes, true));
    jsonConcat(options.json, formatCommands(deviceData.commands));
