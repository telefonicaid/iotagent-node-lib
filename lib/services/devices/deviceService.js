...
function createInitialEntity(deviceData, newDevice, callback) {
    ...
    if (config.getConfig().authentication && config.getConfig().authentication.enabled) {
        config.getGroupRegistry().find(deviceData.service, deviceData.subservice, function(error, deviceGroup) {
          var typeInformation;
          if (deviceGroup) {
              typeInformation = deviceGroup;
          } else if (config.getConfig().types && config.getConfig().types[deviceData.resource]){
              typeInformation = config.getConfig().types[deviceData.resource];
          } else if (config.getConfig().types && config.getConfig().types[deviceData.type]){
              typeInformation = config.getConfig().types[deviceData.type];
          } else {
              typeInformation = deviceData;
          }
          ...
          if (typeInformation && typeInformation.trust) {
              async.waterfall([
                  ...
                  apply(intitialEntityRequest, deviceData, newDevice)
              ], callback);
          } else {
        ...
        intitialEntityRequest(deviceData, newDevice, options, callback);
    }

}

function intitialEntityRequest(deviceData, newDevice, options, callback){
