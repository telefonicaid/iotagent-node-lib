    ...
    function prepareDeviceData(deviceObj, configuration, callback) {
        ...
        if (!deviceData.name) {
            if (configuration && configuration.entityNameExp !== undefined) {
                // TBD: The context for the JEXL expression should be the ID, TYPE, S, SS and avaliable measures.
                let attrList = [
                    { name: 'id', type: 'String', value: deviceData.id },
                    { name: 'type', type: 'String', value: deviceData.type },
                    { name: 'service', type: 'String', value: deviceData.service },
                    { name: 'subservice', type: 'String', value: deviceData.subservice }
                ];
                ...
                deviceData.name = expressionPlugin.applyExpression(configuration.entityNameExp, ctxt, deviceData);
            } else {
                let conjunction;
