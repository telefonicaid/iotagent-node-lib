...
function registerDevice(deviceObj, callback) {
    ...
    function completeRegistrations(error, processResults) {
        if (error) {
            return callback(error);
        }

        logger.debug('Registering device into NGSI Service:\n%s', JSON.stringify(processResults[1], null, 4));

        async.waterfall([
            apply(registrationUtils.sendRegistrations, false, processResults[1]),
            apply(processContextRegistration, processResults[1]),
            apply(createInitialEntity, processResults[1]),
...
function unregisterDevice(id, service, subservice, callback) {
    ...
    config.getRegistry().get(id, service, subservice, function(error, device) {
        async.waterfall([
            ...
            async.apply(config.getRegistry().remove, id, service, subservice)
        ], callback);
    });
}
...
function updateRegisterDevice(deviceObj, callback) {
    ...
    async.waterfall([
        async.apply(config.getRegistry().get, deviceObj.id, deviceObj.service, deviceObj.subservice),
