...
function registerDevice(deviceObj, callback) {
    ...
    for (var i = 0; i < fields.length; i++) {
        if (config.getConfig().types[deviceData.type] && config.getConfig().types[deviceObj.type][fields[i]]) {
            deviceData[fields[i]] =
                (deviceData[fields[i]]) ? deviceData[fields[i]] : config.getConfig().types[deviceObj.type][fields[i]];
        } else {
...
function unregisterDevice(id, callback) {
    ...
    registry.get(id, function(error, device) {
        async.waterfall([
            async.apply(removeAllSubscriptions, device),
            processUnsubscribes,
            async.apply(sendRegistrations, true, device),
 ...
 */
function init(newRegistry, newGroupRegistry, callback) {
