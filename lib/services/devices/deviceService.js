...
function registerDevice(deviceObj, callback) {
    ...
    function prepareDeviceData(deviceObj, configuration, callback) {
        ...
        if (!deviceData.name) {
            //check if we should evaluate am expression (JUST JEXL with basic context id, type, Service, Subservice)
            if (deviceData.entity_name_exp) {
                deviceData.name = jexlParser.applyExpression(deviceData.entity_name_exp, deviceData, {});
            }
            if (deviceData.name === undefined || deviceData.name === null) {
                let conjunction;
