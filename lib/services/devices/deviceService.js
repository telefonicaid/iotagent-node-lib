    ...
    function completeRegistrations(error, deviceData) {
        ...
        async.waterfall([
            ...
            if (error) {
                ...
                if ('commandLegacy' in deviceData && deviceData.commandLegacy !== undefined) {
                    deviceObj.commandLegacy = deviceData.commandLegacy;
                }
...
function updateRegisterDeviceNgsi1(deviceObj, callback) {
    ...
    function combineWithNewDevice(newDevice, oldDevice, callback) {
        if (oldDevice) {
            ...
            if ('commandLegacy' in newDevice && newDevice.commandLegacy !== undefined) {
                oldDevice.commandLegacy = newDevice.commandLegacy;
            }
...
function findOrCreate(deviceId, group, callback) {
    getDevice(deviceId, group.service, group.subservice, function(error, device) {
        if (!error && device) {
            ...
            if ('commandLegacy' in group && group.commandLegacy !== undefined) {
                newDevice.commandLegacy = group.commandLegacy;
            }
