...
function findOrCreate(deviceId, group, callback) {
    getDeviceSilently(deviceId, group.service, group.subservice, function (error, device) {
        if (!error && device) {
            ...
            logger.info(context, 'newDevice %j group %j', newDevice, group);
            if (newDevice.autoprovision === undefined || newDevice.autoprovision === true) {
                registerDevice(newDevice, function (error, device) {
                    callback(error, device, group);
                });
            } else {
                logger.info(context, 'Device %j not provisioned due autoprovision is disabled by its conf', newDevice);
