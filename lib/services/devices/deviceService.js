...
function createInitialEntityNgsiLD(deviceData, newDevice, callback) {
    ...
    jsonConcat(json, formatAttributesNgsi2(deviceData.active, false));
    jsonConcat(json, formatAttributesNgsi2(deviceData.staticAttributes, true));
    jsonConcat(json, formatCommandsNgsi2(deviceData.commands));
    ...
    json = ngsiService.formatAsNGSILD(json);
    delete json['@context'];
    ...
    var options = {
        ...
        headers: {
            ...
            'Link': '<'  + config.getConfig().contextBroker.jsonLdContext +
    ...
    executeWithSecurity(options, newDevice, createInitialEntityHandlerNgsiLD(deviceData, newDevice, callback));
}
...
function createInitialEntity(deviceData, newDevice, callback) {
    if (config.checkNgsiLD()) {   
        createInitialEntityNgsiLD(deviceData, newDevice, callback);
...
function updateEntityNgsiLD(deviceData, updatedDevice, callback) {
    var options = {
        url: config.getConfig().contextBroker.url + '/ngsi-ld/v1/entities/' + String(deviceData.name) + '/attrs',
        method: 'POST',
        json: {
        },
        headers: {
    ...
    if (deviceData.cbHost && deviceData.cbHost.indexOf('://') !== -1) {
        options.url = deviceData.cbHost + '/ngsi-ld/v1/entities/' + String(deviceData.name) + '/attrs';
    } else if (deviceData.cbHost && deviceData.cbHost.indexOf('://') === -1) {
        options.url = 'http://' + deviceData.cbHost + '/ngsi-ld/v1/entities/' + String(deviceData.name) + '/attrs';
    }
    ...
    options.json = ngsiService.formatAsNGSILD(options.json);
