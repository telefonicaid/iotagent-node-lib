...
const templateGroup = require('../../templates/deviceConfigGroup.json');
...
const templateConfigGroup = JSON.parse(JSON.stringify(templateGroup));
templateConfigGroup.properties[constants.CONFIGGROUP_TERM] = templateConfigGroup.properties.services;
delete templateConfigGroup.properties.services;
...
function applyConfigurationHandler(newConfiguration, callback) {
    if (configurationHandler && newConfiguration) {
        ...
        } else if (newConfiguration[constants.CONFIGGROUP_TERM]) {
            async.map(newConfiguration[constants.CONFIGGROUP_TERM], configurationHandler, callback);
        } else {
...
function handleCreateDeviceGroup(req, res, next) {
    ...
    if (req.body.services) {
        lenCount = req.body.services.length;
    } else {
        lenCount = req.body[constants.CONFIGGROUP_TERM].length;
        req.body.services = req.body[constants.CONFIGGROUP_TERM];
    }
...
function handleListDeviceGroups(req, res, next) {
    const listHandler = function (error, group) {
        if (error) {
            ...
            } else if (group[constants.CONFIGGROUP_TERM]) {
                translatedGroup[constants.CONFIGGROUP_TERM] = group[constants.CONFIGGROUP_TERM].map(
                    applyMap.bind(null, internalToApi)
                );
            } else {
...
function loadConfigContextRoutes(router) {
    router.post(
        constants.CONFIGGROUP_API_PATH,
        restUtils.checkRequestAttributes('headers', mandatoryHeaders),
