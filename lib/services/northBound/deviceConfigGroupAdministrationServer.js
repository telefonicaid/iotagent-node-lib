...
const groupConfigService = require('./../groups/groupConfigService');
...
function applyConfigurationHandler(newConfiguration, callback) {
    if (configurationHandler && newConfiguration) {
        if (newConfiguration.configGroups) {
...
function handleListDeviceGroups(req, res, next) {
    ...
    if (req.headers['fiware-servicepath'] === '/*') {
        groupConfigService.list(req.headers['fiware-service'], req.query.limit, req.query.offset, listHandler);
    } else {
...
function loadContextRoutes(router) {
    ...
    router.get('/iot/configGroups', restUtils.checkRequestAttributes('headers', mandatoryHeaders), handleListDeviceGroups);
