...
function handleUpdateNgsi2(req, res, next) {
    ...
    if (contextServerUtils.updateHandler || contextServerUtils.commandHandler) {
        ...
        logger.error(context, 'Tried to handle an update request [%j] before the update handler was stablished.', req);
...
function updateErrorHandlingNgsi2(error, req, res, next) {
    ...
    logger.debug(context, 'Update NGSIv2 error [%j] handing request: %j', error, req);
