...
function loadContextRoutesNGSILD(router) {
    ...
    const payload = config.getConfig().server.ldPayloadSupport;
    let i;
    ...
    for (i = 0; i < updatePaths.length; i++) {
        router.patch(updatePaths[i], [
            ...
            preprocessNGSILD,
            validateNGSILD(payload.supportNull, payload.supportDatasetId),
            handleUpdateNgsiLD,
    ...
    for (i = 0; i < overwritePaths.length; i++) {
        router.put(overwritePaths[i], [
            ...
            middlewares.validateJson(updateContextTemplateNgsiLD),
            preprocessNGSILD,
            validateNGSILD(false, payload.supportDatasetId),
            handleUpdateNgsiLD,
