...
function generateMergePatchActionNgsiLD(req, callback) {
    ...
    function cleanNulls(payload){
        console.log( Object.keys(payload))
        Object.keys(payload).forEach((key) =>{
            const value = payload[key];
            if ( value === 'urn:ngsi-ld:null'){
               payload[key] = null; 
            } else if (typeof value === 'object' &&
                !Array.isArray(value) &&
                value !== null){
                payload[key] = cleanNulls(payload[key]);
            }
        })
        return  payload;
    }
    ...
    function getAttributes(device, body){
        const payload = cleanNulls(body || {});
        const keys = Object.keys(payload);
        const attributes = [];
        for (const j in device.commands) {
            if (keys.includes(device.commands[j].name)) {
                const obj = payload[device.commands[j].name]
                if ( obj === null) {
                    attributes.push({
                        type: device.commands[j].type,
                        value: null,
                        name: device.commands[j].object_id
                    });
                } else {
                     attributes.push({
                        type: device.commands[j].type,
...
function handleMergePatchNgsiLD(req, res, next) {
    ...
    if (contextServerUtils.mergePatchHandler){
        return generateMergePatchActionNgsiLD(req, handleMergePatchRequest);
    } else {
...
function loadContextRoutesNGSILD(router) {
    ...
    router.patch('/ngsi-ld/v1/entities/:entity', [handleMergePatchNgsiLD, mergePatchErrorHandlingNgsiLD]);

    // Overwrite/PUT endpoints
