 ...
 *
 * @param {Boolean} rejectNull          Whether to reject NGSI-LD nulls in the payload
 * @param {Boolean} rejectDatasetId     Whether to reject multiattributes in the payload.
 * @return {Object}            Express middleware used in request validation.
 */
function validateNGSILD(rejectNull, rejectDatasetId) {
    return function validate(req, res, next) {
        if (rejectNull && res.locals.hasNulls) {
            next(new errors.BadRequest('This IoT Agent does not support NGSI-LD Null within the payload for this endpoint.'));
        } else if (rejectNull && res.locals.hasDatasetId) {
...
function generateMergePatchActionNgsiLD(req, callback) {
    ...
    function getAttributes(device, body){
        const keys = Object.keys(body);
        const attributes = [];
        for (const j in device.commands) {
            if (keys.includes(device.commands[j].name)) {
                const obj = body[device.commands[j].name]
                if ( obj === null) {
                    attributes.push({
                        type: device.commands[j].type,
                        value: null,
        ...
        function (error, deviceObj) {
            if (error) {
                ...
                const executeMergePatchHandler = apply(
                    ...
                    contextServerUtils.getLDTenant(req),
                    contextServerUtils.getLDPath(req),
                    getAttributes(deviceObj, req.body)
                );
...
/**
 * Error handler for NGSI-LD merge-patch requests.
 *
 * @param {Object} error            Incoming error
 * @param {Object} req              Request that was handled in first place.
 * @param {Object} res              Response that will be sent.
 */
function mergePatchErrorHandlingNgsiLD(error, req, res, next) {
    let code = 500;

    logger.debug(context, 'merge-patch NGSI-LD error [%s] handling request: %s', error.name, error.message);
...
function loadContextRoutesNGSILD(router) {
    ...
    const rejectDatasetId = false;//config.getConfig().rejectDatasetId;

    let i;

    logger.info(context, 'Loading NGSI-LD Context server routes');
    // Update Patch endpoints - this endpoint accepts NGSI-LD Null
    for (i = 0; i < updatePaths.length; i++) {
        router.patch(updatePaths[i], [
            ...
            validateNGSILD(false, rejectDatasetId),
            preprocessNGSILD,
            handleUpdateNgsiLD,
            updateErrorHandlingNgsiLD
        ]);
    }
    // Merge Patch endpoints - this endpoint accepts NGSI-LD Null
    router.patch('/ngsi-ld/v1/entities/:entity', [
        preprocessNGSILD,
        validateNGSILD(false, rejectDatasetId),
        handleMergePatchNgsiLD, 
        mergePatchErrorHandlingNgsiLD
    ]);
    ...
    for (i = 0; i < overwritePaths.length; i++) {
        router.put(overwritePaths[i], [
            ...
            middlewares.validateJson(updateContextTemplateNgsiLD),
            preprocessNGSILD,
            validateNGSILD(true, rejectDatasetId),
            handleUpdateNgsiLD,
            updateErrorHandlingNgsiLD
        ]);
    }
