...
function createUpdateResponse(req, res, results) {
    ...
    logger.debug('Generated update response: %j', result);
...
function createQueryResponse(req, res, results) {
    ...
    logger.debug('Generated query response: %j', result);
...
function handleUpdate(req, res, next) {
    ...
    if (updateHandler) {
        ...
        async.waterfall([
            ...
            if (error) {
                logger.debug('There was an error handling the update action: %s.', error);

                next(error);
            } else {
                logger.debug('Update action from [%s] handled successfully.', req.get('host'));
        ...
        logger.error('Tried to handle an update request before the update handler was stablished.');
 ...
 */
function defaultQueryHandler(id, type, attributes, callback) {
    var contextElement = {
        type: type,
        isPattern: false,
        id: id,
    ...
    deviceService.getDeviceByName(id, function(error, ngsiDevice) {
        if (error) {
            callback(error);
        } else {
...
function handleQuery(req, res, next) {
    ...
    function completeAttributes(attributes, device, callback) {
        if (attributes && attributes.length !== 0) {
            logger.debug('Handling received set of attributes: %j', attributes);
            callback(null, attributes);
        } else if (device.lazy) {
            logger.debug('Handling stored set of attributes: %j', attributes);
            callback(null, device.lazy.map(getName));
        } else {
            logger.debug('Couldn\'t find any attributes. Handling with null reference');
            callback(null, null);
        }
    }
    ...
    function handleQueryContextRequests(error, result) {
        if (error) {
            logger.debug('There was an error handling the query: %s.', error);
            next(error);
        } else {
            logger.debug('Query from [%s] handled successfully.', req.get('host'));
...
function handleNotification(req, res, next) {
    ...
    function handleNotificationRequests(error) {
        if (error) {
            logger.error('Error found when processing notification: %j', error);
        }
...
function loadContextRoutes(router) {
    ...
    logger.info('Loading NGSI Contect server routes');
