...
/**
 * Express middleware to manage incoming UpdateContext requests using NGSIv2.
 */
function handleUpdateNgsi2(req, res, next) {
    ...
    if (updateHandler || commandHandler) {
        ...
        async.waterfall([
            apply(generateUpdateActionsNgsi2, req, req.body),
            reduceActions,
            async.series
...
/**
 * Express middleware to manage incoming QueryContext requests using NGSIv2.
 */
function handleQueryNgsi2(req, res, next) {
    ...
    function createQueryRequest(attributes, contextEntity, callback) {
        ...
        async.waterfall([
            apply(
                ...
                req.headers['fiware-service'],
                req.headers['fiware-servicepath']),
        ], function handleFindDevice(error, innerDevice) {
            if (!innerDevice) {
                callback(new errors.DeviceNotFound(contextEntity.id));
            }

            if(!contextEntity.type) {
                contextEntity.type = innerDevice.type;
            }

            deviceService.findConfigurationGroup(innerDevice, function(error, device) {
                var executeCompleteAttributes = apply(
                    completeAttributes,
                    attributes,
                    device
    ...
    var queryAtts = [];
    if (req.params.attr) {
        queryAtts = [req.params.attr];
    } else if(req.query.attrs) {
        queryAtts = req.query.attrs.split(',');
    }
...
function loadContextRoutes(router) {
    ...
    var updateMiddlewaresNgsi1 = [
        ...
        queryPathsNgis2 = [
            '/v2/entities',
            '/v2/entities/:entity',
            '/v2/entities/:entity?attrs=:attrList',
    ...
    if (config.checkNgsi2()) {
        ...
        for (i = 0; i < queryPathsNgis2.length; i++) {
            router.get(queryPathsNgis2[i], queryMiddlewaresNgsi2);
        }
        router.post('/notify', [
            middlewares.ensureType,
