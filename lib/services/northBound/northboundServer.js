...
function openmetricsHandler(req, res) {
    statsRegistry.getAllGlobal(function (err, globalStats) {
        ...
        // Workaround for express.accepts not recognizing `version=...;'
        // in the Accept header, which is used by openmetrics collectors
        if (req.headers.accept && req.headers.accept.contains('version=1.0.0')) {
            acceptVersion = '1.0.0';
            req.headers.accept = req.headers.accept.replace(/version=1.0.0;?/, '');
        }
