...
/**
 * Predefined http handler that returns current prometheus metrics
 */
/* eslint-disable-next-line  no-unused-vars */
function prometheusHandler(req, res) {
    statsRegistry.getAllGlobal(function (err, globalStats) {
        ...
        for (const key in globalStats) {
            if (hasOwnProperty(globalStats, key)) {
                metrics.push('# HELP ' + key);
...
function start(config, callback) {
    ...
    northboundServer.router.get('/metrics', prometheusHandler);
