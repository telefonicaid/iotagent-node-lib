...
function openmetricsHandler(req, res) {
    // Content-Type MUST BE 'application/openmetrics-text; version=1.0.0; charset=utf-8'
    // for openmetrics collectors, see:
    //https://github.com/OpenObservability/OpenMetrics/blob/main/specification/OpenMetrics.md#overall-structure
    // However, it should be 'text/plain; version=0.0.4; charset=utf-8'
    // for prometheus.compatible collectors, see:
    // https://github.com/prometheus/docs/blob/main/content/docs/instrumenting/exposition_formats.md
    let contentType = 'application/openmetrics-text; version=1.0.0; charset=utf-8';
    // To differentiate both kinds of collectors, the `Accept` header is used.
    // Openmetrics-based collectors use an `Accept` header such as:
    //   Accept: application/openmetrics-text; version=1.0.0; charset=utf-8';
    // See https://github.com/open-telemetry/opentelemetry-collector-contrib/issues/18913
    // However, this is sort of a prometheus-specific extension to the `Accepts`
    // header, that breaks the regexps in `express.accepts`.
    ...
    if (charset && charset != 'utf-8') {
        logger.error(context, 'Unsupported charset: %s', charset);
    ...
    switch (req.accepts(['text/plain', 'application/openmetrics-text'])) {
        case 'application/openmetrics-text':
            ...
            if (version && version != '1.0.0') {
                logger.error(context, 'Unsupported openmetrics version: %s', version);
        ...
        case 'text/plain':
            ...
            break;
        default:
            logger.error(context, 'Unsupported accept header: %s', req.headers.accept);
            res.status(406).send('Unsupported accept header');
            return;
    }
    statsRegistry.getAllGlobal(function (err, globalStats) {
        if (err) {
            logger.error(context, 'Error retrieving global stats: %s', err);
            res.status(500).send('Error retrieving global stats');
            return;
        }
...
function start(config, callback) {
    ...
    northboundServer.router.get('/metrics', openmetricsHandler);
