...
function openmetricsHandler(req, res) {
    statsRegistry.getAllGlobal(function (err, globalStats) {
        ...
        // unless the collector requests openmetrics text format with Accept header,
        // in which case it MUST BE 'application/openmetrics-text; version=1.0.0; charset=utf-8'
        let contentType = 'text/plain; version=0.0.4; charset=utf-8';
        // Workaround for `express.accepts` not supporting generic key-value pairs
        // (other then ;q=...) in the Accept headers.
        const kvPairs = {};
        if (req.headers.accept) {
            ...
            for (let i = 0; i < parts.length; i++) {
                ...
                // any `key=value` pair not matching the standard
                // `q=...` format, goes to `kvPairs`
                if (trimmed.contains('=') && !trimmed.startswith('q=')) {
            ...
            req.headers.accept = regular.join(';');
        }
        switch (req.accepts(['text/plain', 'application/openmetrics-text'])) {
            case 'application/openmetrics-text':
                if (kvPairs.version && kvPairs.version != '1.0.0') {
