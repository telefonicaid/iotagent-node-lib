...
function handleUpdateDevice(req, res, next) {
    function applyUpdatingHandler(device, callback) {
        if (updatingHandler) {
            logger.debug(context, 'Apply updating handler: %j', device);
        ...
        deviceService.getDevice(
            ...
            function (error, device) {
                if (error) {
                    ...
                    async.waterfall([apply(applyUpdatingHandler, newDevice)], function f() {
                        logger.debug(context, 'device: %j', device);
                        deviceService.updateRegister(newDevice, isTypeOrNameUpdated, function handleDeviceUpdate(
                            error
                        ) {
                            if (error) {
                                next(error);
