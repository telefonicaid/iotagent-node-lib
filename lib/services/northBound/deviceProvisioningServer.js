...
function handleProvision(req, res, next) {
    function handleProvisioningFinish(error, results) {
        if (error) {
            ...
            logger.debug('Device provisioning request succeeded');
            res.status(201).json({});
        }
    }
...
function toProvisioningAPIFormat(device) {
    ...
    return {
        ...
        commands: device.commands.map(attributeToProvisioningAPIFormat),
...
function handleGetDevice(req, res, next) {
    deviceService.getDevice(req.params.deviceId, req.headers['fiware-service'], req.headers['fiware-servicepath'], function(error, device) {
        if (error) {
            next(error);
        } else if (device) {
...
function handleRemoveDevice(req, res, next) {
    statsRegistry.add('deviceRemovalRequests', 1, function() {
        deviceService.unregister(req.params.deviceId, req.headers['fiware-service'], req.headers['fiware-servicepath'], function(error) {
            if (error) {
                next(error);
            } else {
                res.status(204).send();
