...
var restUtils = require('./restUtils'),
    groupconfigService = require('./../groups/groupConfigService'),
    async = require('async'),
    apply = async.apply,
    templateconfigGroup = require('../../templates/deviceConfigGroup.json'),
...
function handleCreateDeviceGroup(req, res, next) {
    ...
    for (var i = 0; i < req.body.configGroups.length; i++) {
    ...
    async.series([
        apply(groupconfigService.create, req.body),
...
function handleListDeviceGroups(req, res, next) {
    ...
    if (req.headers['fiware-servicepath'] === '/*') {
        groupconfigService.list(
            req.headers['fiware-service'],
            req.query.limit,
            req.query.offset,
            listHandler);
    } else {
        groupconfigService.find(req.headers['fiware-service'], req.headers['fiware-servicepath'], null, listHandler);
    }
}
...
function handleModifyDeviceGroups(req, res, next) {
    ...
    async.series([
        apply(groupconfigService.update, req.headers['fiware-service'], req.headers['fiware-servicepath'],
...
function handleDeleteDeviceGroups(req, res, next) {
    ...
    function getGroup (req, callback) {
        groupconfigService.get(req.query.resource, req.query.apikey, function (error, group){
            if (error) {
                callback(error);
            } else{
                callback(null, group);
    ...
    function deleteGroup (req, groupToDelete, callback) {
        groupconfigService.remove(req.headers['fiware-service'], req.headers['fiware-servicepath'],
        req.query.resource, req.query.apikey, req.query.device, function (error){
            if (error) {
                callback(error);
            } else{
    ...
    async.waterfall([
        apply(getGroup, req),
        apply(deleteGroup, req),
        applyRemoveConfigurationHandler],
        function(error) {
            if (error) {
                next(error);
            } else {
