...
function handleModifyDeviceGroups(req, res, next) {
    async.series([
        apply(groupService.update, req.query.resource, req.query.apikey, req.body),
        apply(applyConfigurationHandler, req.body)
    ], function(error) {
        if (error) {
            next(error);
...
function handleDeleteDeviceGroups(req, res, next) {
    groupService.remove(req.query.resource, req.query.apikey, function(error) {
        if (error) {
            next(error);
        } else {
            res.status(200).send({});
