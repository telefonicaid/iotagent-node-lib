...
function applyMiddlewares(entity, callback) {
    function emptyMiddleware(callback) {
        callback(null, entity);
    }

    if (updateMiddleware && updateMiddleware.length > 0) {
        var middlewareList = _.clone(updateMiddleware);
...
function sendUpdateValue(entityName, attributes, typeInformation, token, callback) {
    ...
    async.waterfall([
        ...
        apply(applyMiddlewares, payload)
