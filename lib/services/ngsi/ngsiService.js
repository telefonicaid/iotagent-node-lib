        ...
        if (error) {
            ...
            if (result) {
                ...
                if (result instanceof Array) {
                    ...
                    if (config.getConfig().timestamp && !utils.isTimestampedNgsi2(result)) {
...
function updateTrust(deviceGroup, deviceInformation, trust, response, callback) {
    if (deviceGroup && response && response.body) {
        var body = JSON.parse(response.body);
        if (body && body.refresh_token) {
            deviceGroup.trust = body.refresh_token;
            config.getGroupRegistry().update(deviceGroup.id, deviceGroup, function(error, cb) {
...
function executeWithDeviceInformation(operationFunction) {
    ...
    return function(entityName, type, apikey, attributes, deviceInformation, callback) {
        logger.debug(context, 'executeWithDeviceInfo entityName %s type %s apikey %s attributes %j deviceInformation %j',
