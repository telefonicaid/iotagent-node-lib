...
function executeWithDeviceInformation(operationFunction) {
    return function (entityName, type, apikey, attributes, deviceInformation, callback) {
        ...
        const service = deviceInformation.service;
        const subservice = deviceInformation.subservice;
        config.getGroupRegistry().findType(service, subservice, currentType, function (error, deviceGroup) {
            let typeInformation;
