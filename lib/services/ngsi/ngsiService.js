...
function convertNGSIv2ToLD(attr){
    ...
    switch (attr.type.toLowerCase()) {
        ...
        case 'datetime':
            obj.value = { 
                '@type': 'DateTime', 
                '@value': moment(attr.value).utc().toISOString()};
            break;
        case 'date':
            obj.value = { 
                '@type': 'Date', 
                '@value': moment(attr.value).utc().format(moment.HTML5_FMT.DATE)};
            break;
        case 'time':
            obj.value = {
                '@type': 'Time', 
                '@value': moment(attr.value).utc().format(moment.HTML5_FMT.TIME_SECONDS)};
            break;
        ...
        case 'geo:multipolygon':
            obj.type = 'GeoProperty';
            obj.value = { type: 'Point', coordinates: attr.value};
            break;
           ...
           switch (key) {
                case constants.TIMESTAMP_ATTRIBUTE:
                    ...
                    if(timestamp ===  constants.ATTRIBUTE_DEFAULT || !(moment(timestamp).isValid())){
                        ...
                        obj.observedAt = moment(timestamp).utc().toISOString();
                    }
                    break;
                default:
    ...
    async.waterfall([
        ...
        if (error) {
            ...
            try {
                ...
                callback(new errors.BadGeocoordinates(JSON.stringify(payload)));
            }
...
function sendUpdateValue(entityName, attributes, typeInformation, token, callback) {
    if (config.checkNgsiLD()) {
        sendUpdateValueNgsiLD(entityName, attributes, typeInformation, token, callback);
