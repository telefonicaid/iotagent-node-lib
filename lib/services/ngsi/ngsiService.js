 ...
 * array should comply to the NGSI20's attribute format.
 *
 ...
 */
function sendUpdateValueNgsi20(entityName, attributes, typeInformation, token, callback) {
    var payload = {};
...
/**
 * Makes an update in the Device's entity in the context broker, with the values given in the 'attributes' array. This
 * array should comply to the NGSI10's attribute format.
 *
 * @param {String} entityName         Name of the entity to register.
 * @param {Array} attributes        Attribute array containing the values to update.
 * @param {Object} typeInformation  Configuration information for the device.
 * @param {String} token            User token to identify against the PEP Proxies (optional).
 */
function sendUpdateValueNgsi10(entityName, attributes, typeInformation, token, callback) {
...
function sendUpdateValue(entityName, attributes, typeInformation, token, callback) {
    if (checkNgsi2(config.getConfig())) {
        sendUpdateValueNgsi20(entityName, attributes, typeInformation, token, callback);
    } else {
        sendUpdateValueNgsi10(entityName, attributes, typeInformation, token, callback);
    }
}
