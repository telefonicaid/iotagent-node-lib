...
function applyMiddlewares(middlewareCollection, entity, callback) {
...
function sendUpdateValue(entityName, attributes, typeInformation, token, callback) {
    ...
    async.waterfall([
        ...
        apply(applyMiddlewares, updateMiddleware, payload)
        ...
        generateNGSIOperationHandler('query', entityName, typeInformation, token, options,
            function(error, result) {
                if (error) {
                    ...
                    applyMiddlewares(queryMiddleware, result, callback);
                }
            }));
}
