...
function generateNGSIOperationHandler(operationName, entityName, typeInformation, token, options, callback) {
    return function(error, response, body) {
        if (error) {
            logger.debug('Error found executing ' + operationName + ' action in Context Broker: %s', error);

            callback(error);
        } else if (body && body.orionError) {
            logger.debug('Orion error found executing ' + operationName + ' action in Context Broker: %j',
                body.orionError);
            ...
            logger.debug('Received the following request from the CB:\n\n%s\n\n', JSON.stringify(body, null, 4));

            if (errorField) {
                logger.error('Operation ' + operationName + ' error connecting to the Context Broker: %j', errorField);
            ...
            logger.debug('Access forbidden executing ' + operationName + ' operation');
            callback(new errors.AccessForbidden(
                token,
                options.headers['fiware-service'],
                options.headers['fiware-servicepath']));
        } else {
            logger.debug('Unknown error executing ' + operationName + ' operation');
...
function sendQueryValue(entityName, attributes, typeInformation, token, callback) {
    ...
    logger.debug('Querying values of the device in the Context Broker at [%s]', options.url);
    logger.debug('Using the following request:\n\n%s\n\n', JSON.stringify(options, null, 4));
