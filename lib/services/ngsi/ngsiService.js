...
function executeWithDeviceInformation(operationFunction) {
    return function (entityName, type, apikey, attributes, deviceInformation, callback) {
        ...
        const currentApikey = apikey ? apikey : deviceInformation.apikey;
        config.getGroupRegistry().getSilently(deviceInformation.resource, currentApikey, function (error, deviceGroup) {
            let typeInformation;
