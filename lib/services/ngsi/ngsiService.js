...
const ngsiv1 = require('./entities-NGSI-v1');
const ngsiv2 = require('./entities-NGSI-v2');
const ngsiLD = require('./entities-NGSI-LD');
...
function sendUpdateValue(entityName, attributes, typeInformation, token, callback) {
    if (config.checkNgsiLD()) {
        ngsiLD.sendUpdateValue(entityName, attributes, typeInformation, token, callback);
    } else if (config.checkNgsi2()) {
        ngsiv2.sendUpdateValue(entityName, attributes, typeInformation, token, callback);
    } else {
        ngsiv1.sendUpdateValue(entityName, attributes, typeInformation, token, callback);
    }
}
...
function sendQueryValue(entityName, attributes, typeInformation, token, callback) {
    if (config.checkNgsiLD()) {
        ngsiLD.sendQueryValue(entityName, attributes, typeInformation, token, callback);
    } else if (config.checkNgsi2()) {
        ngsiv2.sendQueryValue(entityName, attributes, typeInformation, token, callback);
    } else {
        ngsiv1.sendQueryValue(entityName, attributes, typeInformation, token, callback);
    }
}
