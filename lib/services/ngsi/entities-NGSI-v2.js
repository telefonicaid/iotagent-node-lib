...
function sendUpdateValueNgsi2Refactored(entityName, attributes, typeInformation, token, callback) {
    ...
    logger.debug(context, 'sendUpdateValueNgsi2Refactored \n current payload=%j', payload);

    // TBD loop entities
    // Loop for each attribute
    // TBD: apply all plugins: attributeAlias,
    //                         addEvents
    //                         compressTimestamp
    //                         expression
    //                         multientity
    ...
    for (let i = 0; i < payload.entities.length; i++) {
        let currentEntity = payload.entities[i];
        for (const j in currentEntity) {
            // currentEntity.hasOwnProperty(j) // is this check redundant ?
            const attr = Object.assign({}, currentEntity[j]);
            // discard id and type
            if (j !== 'id' || j !== 'type') {
                // Apply Alias: object_id in attributes are in typeInformation.active (TBD lazy and comands)
                let attrAlias = typeInformation.active.find((obj) => {
                    return obj.object_id === j;
                });
                if (attrAlias) {
                    payload.entities[0][attrAlias.name] = payload.entities[0][j];
                    delete payload.entities[0][j];
                } else {
                    attrAlias = typeInformation.active.find((obj) => {
                        return obj.name === j;
                    });
                }
                if (attrAlias && attrAlias.type) {
                    payload.entities[0][attrAlias.name].type = attrAlias.type;
                }
            }
            // ...
        }
    ...
    // ) {
    //     // jshint maxdepth:5
    //     if (!utils.isTimestampedNgsi2(payload)) {
    //         payload = addTimestampNgsi2(payload, typeInformation.timezone);
    //         // jshint maxdepth:5
    //     } else if (!utils.IsValidTimestampedNgsi2(payload)) {
    //         logger.error(context, 'Invalid timestamp:%s', JSON.stringify(payload));
