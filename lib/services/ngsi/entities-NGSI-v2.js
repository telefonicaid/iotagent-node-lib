...
function removeHiddenAttrsFromMultiEntity(entities, typeInformation) {
    function filterForEntity(entity, explicitAttrsList, typeInformation) {
        let attr;
        let effectiveList = [];
        for (let i in explicitAttrsList) {
            attr = explicitAttrsList[i];
            if (typeof attr === 'string') {
                effectiveList.push(attr);
            } else if (typeof attr === 'object' && attr.object_id) {
                //find objectId in active attributes
                let activefromobj_id = {};
                for (let j in typeInformation.active) {
                    if (typeInformation.active && typeInformation.active[j].object_id === attr.object_id) {
                        activefromobj_id = typeInformation.active[j];
                        continue; //object_id must be unique
                    }
                }
                //if name is an expression it could fail.
                if (
                    activefromobj_id.name &&
                    activefromobj_id.entity_name &&
                    entity.id === activefromobj_id.entity_name
                ) {
                    effectiveList.push(activefromobj_id.name);
                }
            }
        }
