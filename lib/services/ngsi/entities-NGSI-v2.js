...
function sendUpdateValueNgsi2(entityName, measures, typeInformation, token, callback) {
    ...
    if (mustInsertTimeInstant) {
        ...
        //search for a Timestamp mapped in an attribute
        if (typeInformation && typeInformation.active) {
            timestampAttrs = typeInformation.active.filter(
                (item) => item.name === constants.TIMESTAMP_ATTRIBUTE && item.object_id !== null
            );
        }

        if (timestampAttrs && timestampAttrs.length > 0 && plainMeasures[timestampAttrs[0]['object_id']]) {
            // Maybe TimeInstant is a mapped attribute, but just the first one
            if (timestampAttrs[0]['expression']) {
                timestamp.value = expressionPlugin.applyExpression(
                    timestampAttrs[0]['expression'],
                    jexlctxt,
                    typeInformation
                );
            } else if (moment(plainMeasures[timestampAttrs[0]['object_id']], moment.ISO_8601, true).isValid()) {
                timestamp.value = plainMeasures[timestampAttrs[0]['object_id']];
            } else {
                callback(
                    new errors.BadTimestamp(plainMeasures[timestampAttrs[0]['object_id']], entityName, typeInformation)
                );
                return;
            }
        } else if (plainMeasures[constants.TIMESTAMP_ATTRIBUTE]) {
