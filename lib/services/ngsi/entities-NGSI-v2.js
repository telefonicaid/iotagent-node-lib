...
function sendUpdateValueNgsi2Refactored(entityName, attributes, typeInformation, token, callback) {
    ...
    if (
                ...
                if (attr.expression) {
                    if (attr.object_id) {
                        ...
                        payload.entities[0][attr.name] = {
                            value: payload.entities[0][attr.name] ? payload.entities[0][attr.name].value : null,
                            type: attr.type
                            //object_id: attr.name
                        };
                    }
        ...
        if (typeof typeInformation.explicitAttrs === 'string') {
            ...
            // TBD: Add measures which are defined attributes
            typeInformation.active.forEach((attr) => {
    ...
    //         logger.debug(context, 'Still no timestamped payload: %j', payload);
