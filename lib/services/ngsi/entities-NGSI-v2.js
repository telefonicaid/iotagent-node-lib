...
function sendUpdateValueNgsi2(entityName, measures, typeInformation, token, callback) {
    ...
    for (let currentAttr of preprocessedAttr) {
        ...
        //Not Agree
        //PRE-Populate de JEXLcontext (except for null or NaN we preffer undefined)
        if (plainMeasures[currentAttr.object_id] !== undefined) {
    ...
    for (ename in entities) {
        for (etype in entities[ename]) {
            ...
            for (attr of entities[ename][etype]) {
                ...
                if (!isEmpty && mustInsertTimeInstant) {
                    //|| plainMeasures[constants.TIMESTAMP_ATTRIBUTE] !== undefined)) {
