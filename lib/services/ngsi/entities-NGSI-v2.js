...
function sendUpdateValueNgsi2Refactored(entityName, attributes, typeInformation, token, callback) {
    ...
    //                         compressTimestamp: WORK IN progress
    ...
    // Sort currentEntity to get first atts without expressions in typeInformation.active:
            ...
            if (attr && (attr.entity_type || attr.entity_name)) {
                ...
                if (
                    ...
                    if (newAttr && newAttr.type === constants.TIMESTAMP_TYPE_NGSI2 && newAttr.value) {
                        if (compressTimestampPlugin.fromBasicToExtended(newAttr.value)) {
                            newAttr.value = compressTimestampPlugin.fromBasicToExtended(newAttr.value);
                        }
                    }
    ...
    if (
        ...
        if (timestampValue) {
            for (let n = 0; n < payload.entities.length; n++) {
                payload.entities[n] = addTimestampNgsi2(payload.entities[n], typeInformation.timezone, timestampValue);
            }
        }
        // jshint maxdepth:5
    ...
    for (let m = 0; m < payload.entities.length; m++) {
        payload.entities[m] = NGSIUtils.castJsonNativeAttributes(payload.entities[m]);
    }
    logger.debug(context, 'sendUpdateValueNgsi2Refactored \n payload with native types %j', payload);

    // // TBD: Prevent default entity is just composed of id and type ?
