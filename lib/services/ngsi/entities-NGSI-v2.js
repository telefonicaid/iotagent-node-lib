...
function sendUpdateValueNgsi2Refactored(entityName, attributes, typeInformation, token, callback) {
    ...
    // Loop for add attrs from type.information.active depending on explicitAttrs
    if (
        ...
        // Loop for add attrs from type.information.active and static into pyaload entities (entity[0])
        if (typeInformation.active) {
            typeInformation.active.forEach((attr) => {
                if (attr.expression) {
                    if (attr.object_id) {
        ...
        if (typeof typeInformation.explicitAttrs === 'string') {
                ...
                typeInformation.active.forEach(function (att) {
                    if (att.expression) {
                        if (expressionPlugin.contextAvailable(att.expression, ctxt, typeInformation)) {
                            let expandedAttr = {
                                name: att.name,
                                value: att.expression, // it doesn't matter final value
                                type: att.type
                            };
            ...
            try {
                ...
                if (selectedAttrs.length === 0) {
                    // TBD: SelectedAttrs === [] what implies ?
                }
            } catch (e) {
                // nothing to do: exception is already logged at info level
            }
            ...
            // Add measures which are defined attributes
            typeInformation.active.forEach((attr) => {
        ...
        // Loop for add seleted attrs from type.information.active and static into pyaload entities (entity[0])
        if (typeInformation.active) {
            typeInformation.active.forEach((attr) => {
                if (selectedAttrs.includes(attr.name)) {
                    if (attr.object_id) {
        ...
        } // TBD: Do not include static atts?
    }
    ...
    //                         compressTimestamp: NOT DONE
    //                         expression: WORK IN progress
    //                         multientity: WORK IN progess
    ...
    // TBD: Sort currentEntity keys by presence of expressions in typeInformation.active:
    logger.debug(context, 'sendUpdateValueNgsi2Refactored \n currentEntity %j ', currentEntity);
    for (const k in currentEntity) {
        if (typeInformation.active) {
            typeInformation.active.forEach(function (att) {
                if (att.object_id && att.object_id === k && att.expression) {
                    let m = currentEntity[k];
                    delete currentEntity[k];
                    currentEntity[k] = m; // to the end
                } else if (att.name && att.name === k && att.expression) {
                    let m = currentEntity[k];
                    delete currentEntity[k];
                    currentEntity[k] = m; // to the end
                }
    ...
    for (const j in currentEntity) {
        ...
        if (j !== 'id' || j !== 'type') {
            ...
            if (attr && attr.expression) {
                ...
                if (!expressionPlugin.checkJexl(typeInformation)) {
                    ...
                    if (
                        ...
                        logger.debug(context, 'sendUpdateValueNgsi2Refactored \n apply expression ELSE');
                    }
                } else {
                    // jexl expression plugin
                    if (isNaN(res)) {
                        // and just ensure res is not string!!!
                        newAttr.value = null;
                    } else if (res === null) {
                        newAttr.value = null;
                    }
                ...
                // Update ctxt with the value for attr for next iteration (next attr): Bad Idea or
                // maybe we should check if attr.name is already in ctxt ?
                // if (!ctxt[attr.name]) {
                //     ctxt[attr.name] = payload.entities[0][attr.name].value;
                // }
            }
            ...
            if (attr && (attr.entity_type || attr.entity_name)) {
                ...
                if (attr.entity_name) {
                    try {
                        if (expressionPlugin.contextAvailable(attr.entity_name, ctxt, typeInformation)) {
                            entityName = expressionPlugin.applyExpression(attr.entity_name, ctxt, typeInformation);
                        } else {
                            logger.warn(
                                context,
                            ...
                            entityName = attr.entity_name;
                        }
                        entityName = entityName ? entityName : attr.entity_name;
                    } catch (e) {
                        logger.error(
                            context,
                ...
                let newEntity = {
                    id: entityName ? entityName : payload.entities[0].id,
                ...
                if (alreadyEntity) {
                    ...
                    if (
                        ...
                        let timestampValue;
                        if (
                            payload.entities[0][constants.TIMESTAMP_ATTRIBUTE] &&
                            payload.entities[0][constants.TIMESTAMP_ATTRIBUTE].value
                        ) {
                            // and check constants.TIMESTAMP_TYPE_NGSI2
                            timestampValue = payload.entities[0][constants.TIMESTAMP_ATTRIBUTE].value;
                            // } else if (newAttr && newAttr.type === constants.TIMESTAMP_TYPE_NGSI2 && newAttr.value) {
                            //     timestampValue = newAttr.value;
                            // } else if (newAttr.metadata && newAttr.metadata[constants.TIMESTAMP_ATTRIBUTE] &&
                            ...
                            //     timestampValue = newAttr.metadata[constants.TIMESTAMP_ATTRIBUTE].value;
                        }
                ...
                // If newAattr is 'TimeInstant' should be add this timestamp to paylaod (entity[0])
                if (
                    'timestamp' in typeInformation && typeInformation.timestamp !== undefined
                        ? typeInformation.timestamp
                        : config.getConfig().timestamp
                    ...
                    if (j === constants.TIMESTAMP_ATTRIBUTE) {
                        if (newAttr && newAttr.type === constants.TIMESTAMP_TYPE_NGSI2 && newAttr.value) {
                            let timestampValue = newAttr.value;
                        ...
                        //            newAttr.metadata[constants.TIMESTAMP_ATTRIBUTE].type === constants.TIMESTAMP_TYPE_NGSI2 &&
                        //            newAttr.metadata[constants.TIMESTAMP_ATTRIBUTE].value) {
                        //     let timestampValue = newAttr.metadata[constants.TIMESTAMP_ATTRIBUTE].value;
                        //     payload.entities[0] = addTimestampNgsi2(payload.entities[0],
                        //                                             typeInformation.timezone,
                        //                                             timestampValue);
                        //     logger.debug(context, 'sendUpdateValueNgsi2Refactored \n newAttr.metadata is TimeInstant and new payload=%j', payload);
                    }
                }
            }
        } // if (j !== 'id' || j !== 'type')
    ...
    if (
        ...
        // jshint maxdepth:5
        if (!utils.isTimestampedNgsi2(payload.entities)) {
    ...
    // Removes object_id keys from payload
    payload.entities.forEach(function (entity) {
        for (var key in entity) {
            if (entity[key] && entity[key].object_id) {
                delete entity[key].object_id;
            }
        }
    });
    logger.debug(context, 'sendUpdateValueNgsi2Refactored \n payload purged of object_id =%j', payload);

    // // Prevent default entity is just composed of id and type
    // if (payload.entities.length > 0 && Object.keys(payload.entities[0]).length === 2 &&
    //     payload.entities[0].hasOwnProperty('id') && payload.entities[0].hasOwnProperty('type')
    //    ) {
    //     //payload.entities.shift();
    // }
    // logger.debug(context, 'sendUpdateValueNgsi2Refactored \n payload purged of first empty entity =%j', payload);
    options.json = payload;
    ...
    if (Object.keys(options.json).length > 0) {
        ...
        if (Object.keys(options.json.entities).length === 1) {
            ...
            url = '/v2/entities/' + payload.entities[0].id + '/attrs';
            if (typeInformation && typeInformation.type) {
                //url += '?type=' + typeInformation.type;
                url += '?type=' + payload.entities[0].type;
            }
