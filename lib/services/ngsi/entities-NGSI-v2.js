...
function sendUpdateValueNgsi2(entityName, measures, typeInformation, token, callback) {
    ...
    let timestamp = { type: constants.TIMESTAMP_TYPE_NGSI2 }; //timestam attr for insertions. Value will be added later
    let plainMeasures = null;
    ...
    if (measures !== undefined && measures instanceof Array) {
        plainMeasures = measures.reduce((result, item) => {
    ...
    //All the static
    if (typeInformation.staticAttributes !== undefined && typeInformation.staticAttributes instanceof Array) {
        jexlctxt = {
            ...jexlctxt,
            ...typeInformation.staticAttributes.reduce((result, item) => {
                result[item.name] = item.value;
                return result;
            }, {})
        };
    ...
    if (idTypeSSSList !== undefined && idTypeSSSList instanceof Array) {
        jexlctxt = {
            ...jexlctxt,
            ...idTypeSSSList.reduce((result, item) => {
                result[item.name] = item.value;
                return result;
            }, {})
        };
    ...
    if (jexlctxt['TimeInstant']) {
        //if it comes from a measure
        if (moment(jexlctxt['TimeInstant'], moment.ISO_8601, true).isValid()) {
            timestamp.value = jexlctxt['TimeInstant'];
        } else {
            callback(new errors.BadTimestamp(null, entityName));
        }
    } else if (!typeInformation.timezone) {
        timestamp.value = new Date().toISOString();
        jexlctxt['TimeInstant'] = timestamp.value;
    } else {
    ...
    //Now we can calculate the EntityName
    ...
    //Calculate ExplictAttrs, we will ask for explicit value later with the most complete JEXL context before attrr
    ...
    //Add Static, Lazy, Command and Actives attr attributes
    ...
    //if (typeInformation && typeInformation.commands) {
    //    preprocessedAttr = preprocessedAttr.concat(typeInformation.commands);
    //}
    if (typeInformation && typeInformation.active) {
        preprocessedAttr = preprocessedAttr.concat(typeInformation.active);
    }

    //Proccess every proto Attribute
    for (let i in preprocessedAttr) {
        let hit = false; //any measure, expressiom or value hitted the attr (avoid propagate "silent attr" with null values )
        currentAttr = preprocessedAttr[i];
        ...
        if (currentAttr.value !== undefined) {
            ...
            //add alias to jexlctxt is not null of NaN (We want undefined cotext instead)
            if (valueExpression !== null && !Number.isNaN(valueExpression)) {
                jexlctxt[currentAttr.name] = valueExpression;
            }
            //remove measures that has been shadowed by an alias (some may be left)
        ...
        //include in the attr (skip and hit)
        ...
        if (mustInsertTimeInstant) {
            ...
            currentAttr.metadata['TimeInstant'] = timestamp;
        }
        ...
        if (hit === true) {
            ...
            //store de New Attributte for sending
            entities[attrEntityName][attrEntityType].push(currentAttr);
        }
    }

    //now we can compute explicit with the complete JexlContext
    let explicit = false;
    if (typeof typeInformation.explicitAttrs === 'string') {
        try {
            logger.debug(context, 'sendUpdateValueNgsi2 selectedAttrs ctxt %j', jexlctxt);
    ...
    //more mesures to add (unnhandled/left mesaures) located in measures, just if explicitt is false
    if (explicit == false) {
        //add to primary entity
        if (Object.keys(measures).length > 0) {
            ...
            if (mustInsertTimeInstant) {
                for (let i in measures) {
                    ...
                    currentMeasure.metadata['TimeInstant'] = timestamp;
                }
        ...
        for (etype in entities[ename]) {
            ...
            for (attr of entities[ename][etype]) {
                if (
                    ...
                    (attr.hitted || attr.hitted === undefined) &&
                    (explicit == true ||
                        explicit == false ||
                        (explicit instanceof Array && explicit.includes(currentAttr.name)))
                ) {
                ...
                if (!isEmpty && (mustInsertTimeInstant || plainMeasures['TimeInstant'] !== undefined)) {
                    //specil test case if TimeInstan came in a meaure
                    e['TimeInstant'] = timestamp;
                }
            }
