...
function sendUpdateValueNgsi2Refactored(entityName, attributes, typeInformation, token, callback) {
    ...
    logger.debug(context, 'sendUpdateValueNgsi2Refactored current payload=%j', payload);
    ...
    for (let i = 0; i < payload.entities.length; i++) {
        ...
        for (const j in currentEntity) {
            // currentEntity.hasOwnProperty(j) // check if this is redundant ?
            const attr = Object.assign({}, currentEntity[j]);
            // discard id and type
            if (j !== 'id' || j !== 'type') {
                // apply alias: object_id in attributes are
                //currentEntity[attr] = '';
            }
            // ...
        }
    ...
    logger.debug(context, 'sendUpdateValueNgsi2Refactored ending  payload=%j', payload);
