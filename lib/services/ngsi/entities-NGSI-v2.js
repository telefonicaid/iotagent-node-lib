...
function sendUpdateValueNgsi2Refactored(entityName, attributes, typeInformation, token, callback) {
    ...
    // This loop adds measure values (attributes) into payload entities (entity[0])
    for (let i = 0; i < attributes.length; i++) {
        if (attributes[i].name && attributes[i].type) {
    ...
    if (
        ...
        if (typeInformation.active) {
            // TBD: Priorize multientity attrs with (entity_name and/or entity_type) ?
            typeInformation.active.forEach((attr) => {
                if (attr.expression) {
                    // just include attrs with expressions to be evaluated
        ...
        if (typeInformation.static) {
            typeInformation.static.forEach((attr) => {
                payload.entities[0][attr.name] = {
                    value: attr.value,
                    type: attr.type
                };
        ...
        let ctxt = expressionPlugin.extractContext([].concat(idTypeSSSList), typeInformation);
        let selectedAttrs = [];
        try {
        ...
        if (j !== 'id' || j !== 'type') {
            ...
            if (attr) {
                if (attr.name !== j) {
                    ...
                    payload.entities[0][attr.name].object_id = payload.entities[0][j].object_id
                        ? payload.entities[0][attr.name].object_id
                        : attr.object_id;
                    // Add alias attr to ctxt
                    ctxt[attr.name] = payload.entities[0][j]['value'];
                    logger.debug(context, 'sendUpdateValueNgsi2Refactored \n procesing attr ctxt %j ', ctxt);
                    delete payload.entities[0][j];
                    logger.debug(context, 'sendUpdateValueNgsi2Refactored \n procesing attr payload %j ', payload);
                }
            } else {
                if (typeInformation.active) {
                    attr = typeInformation.active.find((obj) => {
            ...
            // Apply expression
            if (attr && attr.expression /*&& !payload.entities[0][attr.name].value*/) {
                logger.debug(
                    context,
                ...
                if (!expressionPlugin.checkJexl(typeInformation)) {
                    ...
                    if (
                        ...
                        // Due to expressionBasedTransformations-test.js
                        payload.entities[0][attr.name].value = res;
                        // TBD: add object_id
                        payload.entities[0][attr.name].object_id = attr.object_id;
                        if (
                            payload.entities[0][attr.name].type === 'Number' &&
                            isFloat(payload.entities[0][attr.name].value)
                        ) {
                            payload.entities[0][attr.name].value = Number.parseFloat(
                                payload.entities[0][attr.name].value
                            );
                        } else if (
                            payload.entities[0][attr.name].type === 'Number' &&
                            !Number.isNaN(Number.parseInt(payload.entities[0][attr.name].value))
                        ) {
                            payload.entities[0][attr.name].value = Number.parseInt(
                                payload.entities[0][attr.name].value
                            );
                        } else if (payload.entities[0][attr.name].type === 'Boolean') {
                            payload.entities[0][attr.name].value =
                                payload.entities[0][attr.name].value === 'true' ||
                                payload.entities[0][attr.name].value === '1';
                        } else if (payload.entities[0][attr.name].type === 'None') {
                            payload.entities[0][attr.name].value = null;
                        }
                    } else {
                        logger.debug(
                            context,
                            'sendUpdateValueNgsi2Refactored \n LEGACY remove %j from %j \n',
                            attr.name,
                            payload
                        );
                        // TBD: delete by attr.name and attr.object_id
                        if (
                            payload.entities[0][attr.name].object_id &&
                            payload.entities[0][attr.name].object_id == attr.object_id
                        ) {
            ...
            if (attr && (attr.entity_type || attr.entity_name)) {
                ...
                // TBD: have in to account before remove it several attr has same name but different object_id
                logger.debug(context, 'sendUpdateValueNgsi2Refactored \n MULTI remove %j from %j \n', attr, payload);
                // TBD: delete by attr.name and attr.object_id
                if (
                    payload.entities[0][attr.name].object_id &&
                    payload.entities[0][attr.name].object_id == attr.object_id
    ...
    // TBD: remove object_id keys from payload
    payload.entities.forEach(function (entity) {
        for (var key in entity) {
            if (entity[key].object_id) {
