    ...
    if (typeInformation.explicitAttrs) {
        ...
        if (typeof typeInformation.explicitAttrs === 'boolean') {
            ...
            entities.forEach((entity) => {
                const attsArray = utils.extractAttributesArrayFromNgsiLDEntity(entity);
...
function removeHiddenAttrs(result, typeInformation) {
    ...
    if (typeInformation.explicitAttrs && typeof typeInformation.explicitAttrs === 'boolean') {
        ...
        const attsArray = utils.extractAttributesArrayFromNgsiLDEntity(result);
        const ctx = jexlParser.extractContext(attsArray);
        const res = jexlParser.parse(typeInformation.explicitAttrs, ctx);
...
function sendUpdateValueNgsiLD(entityName, attributes, typeInformation, token, callback) {
    ...
    let url = '/ngsi-ld/v1/entityOperations/upsert/?options=update';
    ...
    for (const j in currentEntity) {
        ...
        if (j !== 'id' || j !== 'type') {
            ...
            if (attr && (attr.entity_type || attr.entity_name)) {
                ...
                let alreadyEntity = payload.find((entity) => {
                ...
                if (newAttr && newAttr.type === constants.TIMESTAMP_TYPE_NGSiLD && newAttr.value) {
                ...
                if (
                    ...
                    newAttr.metadata[constants.TIMESTAMP_ATTRIBUTE].type === constants.TIMESTAMP_TYPE_NGSiLD &&
        ...
        if (timestampValue) {
            ...
            for (let n = 0; n < payload.length; n++) {
                if (!utils.isTimestampedNgsi2(payload[n])) {
                    ...
                    // jshint maxdepth:5
                } else if (!utils.isTimestampedNgsi2(payload[n])) {
                    // legacy check needed?
    ...
    if (
        ...
        (options.json.length > 1 ||
            (options.json.length === 1 && Object.keys(options.json[0]).length > 2)) // more than id and type
    ) {
