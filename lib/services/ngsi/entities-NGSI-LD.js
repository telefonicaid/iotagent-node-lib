...
function removeHiddenAttrsFromMultiEntity(entities, typeInformation) {
    ...
    if (typeInformation.explicitAttrs && typeof typeInformation.explicitAttrs === 'boolean') {
        ...
        entities.forEach((entity) => {
            ...
            var res = jexlParser.parse(typeInformation.explicitAttrs, ctx);
            if (res !== null) {
                explicitAttrsList = explicitAttrsList.concat(res); // res is expected to be an array of string
            } else {
                explicitAttrsList = explicitAttrsList.concat(NGSIUtils.getArrayAttrs(typeInformation.explicitAttrs));
            }
        });
    }
