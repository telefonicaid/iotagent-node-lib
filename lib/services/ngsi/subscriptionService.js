...
const ngsiv1 = require('./subscription-NGSI-v1');
const ngsiv2 = require('./subscription-NGSI-v2');
const ngsiLD = require('./subscription-NGSI-LD');
...
function subscribe(device, triggers, content, callback) {
    if (config.checkNgsiLD()) {
        ngsiLD.subscribe(device, triggers, content, callback);
    } else if (config.checkNgsi2()) {
        ngsiv2.subscribe(device, triggers, content, callback);
    } else {
        ngsiv1.subscribe(device, triggers, content, callback);
    }
}
...
function unsubscribe(device, id, callback) {
    if (config.checkNgsiLD()) {
        ngsiLD.unsubscribe(device, id, callback);
    } else if (config.checkNgsi2()) {
        ngsiv2.unsubscribe(device, id, callback);
    } else {
        ngsiv1.unsubscribe(device, id, callback);
    }
}
