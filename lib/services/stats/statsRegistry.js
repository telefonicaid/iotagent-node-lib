...
function configureDb(callback) {
    if (!config.getConfig().mongodb || !config.getConfig().mongodb.host) {
        logger.fatal('No host found for MongoDB driver.');
        callback(new errors.BadConfiguration('No host found for MongoDB driver'));
    } else {
        var dbName = config.getConfig().mongodb.db,
            port = config.getConfig().mongodb.port || 27017;

        if (!config.getConfig().mongodb.db) {
            dbName = dbService.DEFAULT_DB_NAME;
        }

        dbService.init(config.getConfig().mongodb.host, dbName, port, {}, callback);
    }
}
...
function init(callback) {
    if (config.getConfig().stats && config.getConfig().stats.persistence) {
