...
function openmetricsHandler(req, res) {
    ...
    const supportedVersions = ['1.0.0', '0.0.1'];
    ...
    if (req.headers.accept) {
        const mediaTypes = req.headers.accept.split(',');
        for (let i = 0; i < mediaTypes.length; i++) {
            const parts = mediaTypes[i].split(';');
            let unparsed = [];
            for (let j = 0; j < parts.length; i++) {
