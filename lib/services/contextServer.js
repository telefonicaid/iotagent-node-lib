...
function handleQuery(req, res, next) {
    function addStaticAttributes(id, type, contextElement, callback) {
        ngsi.getDeviceByName(id, function handleFindDevice(error, device) {
            if (error) {
                callback(error);
            } else {
