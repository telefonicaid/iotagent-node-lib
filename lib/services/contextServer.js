...
function generateXmlResponse(template) {
    return function xmlGenerator(response) {
        ...
        for (var i = 0; i < response.contextResponses.length; i++) {
            ...
            queryContextValues.contextResponses += mustache.render(template.contextElementTemplate, contextElementValues);
        }

        console.log('The body:\n\n%s\n\n', JSON.stringify(mustache.render(template.template, queryContextValues), null, 4));
