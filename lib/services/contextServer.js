...
function generateXmlResponse(response) {
    ...
    for (var i = 0; i < response.contextResponses.length; i++) {
        for (var j = 0; j < response.contextResponses[i].contextElement.length; j++) {
            var contextElement = response.contextResponses[i].contextElement[j],
...
function loadTemplates() {
    ...
    async.series([
        async.apply(fs.readFile, './lib/templates/queryContext.xml', 'utf8'),
        async.apply(fs.readFile, './lib/templates/attributeTemplate.xml', 'utf8'),
        async.apply(fs.readFile, './lib/templates/contextResponse.xml', 'utf8')
