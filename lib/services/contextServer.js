...
function createNotificationResponses(req, res, results) {
    var result = {
        contextResponses: []
    };

    for (var i = 0; i < req.body.contextResponses.length; i++) {
        var contextResponse = {
            contextElement: {
                attributes: req.body.contextResponses[i].contextElement.attributes,
                id: req.body.contextResponses[i].contextElement.id,
                isPattern: false,
                type: req.body.contextResponses[i].contextElement.type
            },
            statusCode: {
                code: 200,
                reasonPhrase: 'OK'
    ...
    if (updateHandler) {
        ...
        for (var i = 0; i < req.body.contextResponses.length; i++) {
            updateActions.push(
                async.apply(
                    updateHandler,
                    req.body.contextResponses[i].contextElement.id,
                    req.body.contextResponses[i].contextElement.type,
                    req.body.contextResponses[i].contextElement.attributes)
            );
        }
        ...
        async.series(updateActions, function(error, result) {
            if (error) {
                ...
                res.json(200, createNotificationResponses(req, res, result));
            }
        });
    } else {
        var errorNotFound = new Error({
...
function start(config, callback) {
    ...
    contextServer.app.set('port', config.server.port);
