...
function sendQueryValue(entityName, attributes, typeInformation, token, callback) {
    ...
    request(options, function(error, response, body) {
        if (error) {
            logger.debug(context, 'Error found querying value in Context Broker: %s', error);
            callback(error);
        } else if (body.orionError) {
            logger.debug(context, 'Orion error found querying value in Context Broker: %j', body.orionError);
            callback(new errors.BadRequest(body.orionError.details));
        } else if (response && body && response.statusCode === 200) {
            var errorField = getErrorField(body);

            if (errorField) {
                logger.error(context, 'Query value error connecting to the Context Broker: %j', errorField);
                callback(new errors.BadRequest(JSON.stringify(errorField)));
            } else {
                logger.debug(context, 'Value updated successfully');
                callback(null, body);
            ...
            logger.debug(context, 'Access forbidden querying value');
            callback(new errors.AccessForbidden(
                token,
                options.headers['fiware-service'],
                options.headers['fiware-servicepath']));
        } else {
            logger.debug(context, 'Unknown error querying value');
            callback(new errors.EntityUpdateError(entityName, typeInformation.type));
        }
    });
}
 ...
 *
 * @param {String} entityName           Name of the entity to query/update
 ...
 * @param {Array}  attributes           Attribute array containing the values to update or list of attributes.
