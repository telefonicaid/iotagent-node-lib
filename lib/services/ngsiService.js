...
function getErrorField(body) {
	...
	if(typeof errorField.code !== "undefined" && errorField.code == 200)
		errorField = null;
...
function sendRegistrations(unregister, deviceData, callback) {
    var options = {
        url: (typeof config.contextBroker !== "undefined") 
							? 'http://' + config.contextBroker.host + ':' + config.contextBroker.port + '/NGSI9/registerContext' 
							: 'http://' + config.iotDiscovery.host + ':' + config.iotDiscovery.port + '/ngsi9/registerContext',
        method: 'POST',
        json: {
    ...
    function formatAttributes(originalVector) {
		...
		if(originalVector[i].metadatas){
		    attributeList.push({
		        name: originalVector[i].name,
		        type: originalVector[i].type,
		        isDomain: 'false',
			metadatas: originalVector[i].metadatas
		    });
		}
		else{
		    attributeList.push({
    ...
    request(options, function(error, response, body) {
        if (error) {
			...
			/* fixed an issue with the check of the errorField code. Now the error is not raised if the errorField code is equal to 200 (ReasonPhrase: OK) */

            if (errorField && errorField.code!=200) {
...
function sendUpdateValue(entityName, attributes, typeInformation, token, callback) {
    var cbHost = (typeof config.contextBroker === "undefined") 
							? 'http://' + config.iotBroker.host + ':' + config.iotBroker.port + '/ngsi10/updateContext'
							: 'http://' + config.contextBroker.host + ':' + config.contextBroker.port + '/v1/updateContext',
        options,
        headers = {
	...
	if(typeof config.contextBroker === "undefined"){

		//change attribute key "value" to "contextValue"
		var i;
		for(i = 0; i < attributes.length; i++){
			attributes[i].contextValue = attributes[i]['value'];
			delete attributes[i].value;
		}		
 ...
 */
function updateValue(entityName, resource, apikey, attributes, deviceInformation, callback) {
        ...
        if (config.authentication && config.authentication.enabled) {
            if (typeInformation && typeInformation.trust) {
                async.waterfall([
                    ...
                    apply(sendUpdateValue, entityName, attributes, typeInformation)
                ], callback);
            } else {
                callback(new errors.SecurityInformationMissing(typeInformation.type));
            }
        } else {
            sendUpdateValue(entityName, attributes, typeInformation, null, callback);
        }
    });
}
