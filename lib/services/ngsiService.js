...
function registerDevice(deviceObj, callback) {
    ...
    if (!deviceObj.service || !deviceObj.subservice || !deviceObj.lazy) {
        if (config.types[deviceObj.type]) {
            ...
            var fields = ['service', 'subservice', 'lazy', 'internalAttributes', 'active'];

            for (var i = 0; i < fields.length; i++) {
                deviceData[fields[i]] =
                    (deviceObj[fields[i]]) ? deviceData[fields[i]] : config.types[deviceObj.type][fields[i]];
            }
        } else {
 ...
 */
function sendUpdateValue(deviceId, deviceType, attributes, typeInformation, token, callback) {
    ...
    if (typeInformation) {
        ...
        if (typeInformation.contextBroker) {
            brokerHost = typeInformation.contextBroker.host;
            brokerPort = typeInformation.contextBroker.port;
        }
    }
...
function updateValue(deviceId, deviceType, attributes, deviceInformation, callback) {
    ...
    if (!callback) {
        ...
        typeInformation = config.types[deviceType];
    } else {
    ...
    if (config.authentication && config.authentication.enabled) {
        if (typeInformation && typeInformation.trust) {
            async.waterfall([
                ...
                apply(sendUpdateValue, deviceId, deviceType, attributes, typeInformation)
            ], callback);
        } else {
            callback(new errors.SecurityInformationMissing(deviceType));
        }
    } else {
        sendUpdateValue(deviceId, deviceType, attributes, typeInformation, null, callback);
    }
}
