 ...
 */
function registerDevice(id, type, name, service, subservice, lazyAttributes, callback) {
    var deviceData = {
        id: id,
        type: type,
        name: name,
    ...
    if (!deviceData.name) {
        logger.debug('Device name not found, falling back to device id [%s]', deviceData.id);
        deviceData.name = deviceData.id;
    }

    if (!service || !subservice || !lazyAttributes) {
        if (config.types[type]) {
            logger.debug('Device type [%s] found', type);
            ...
            logger.debug('Device type not found [%s] for device [%s]', type, deviceData.name);
...
function unregisterDevice(id, type, callback) {
    ...
    logger.debug('Unregistering device from NGSI Service');
