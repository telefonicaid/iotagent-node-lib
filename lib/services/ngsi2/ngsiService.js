 ...
 * please contact with::daniel.calvo@atos.net
 */
...
function generateNGSIOperationHandler(operationName, entityName, typeInformation, token, options, callback) {
    return function(error, response, body) {
        ...
        if (error) {
            ...
            || (operationName === 'query' && body !== undefined && (response.statusCode === 200 || response.statusCode === 204) ))) {
...
*  @param {Boolean} updateFlag               If present, operation is update. If not, query
 * @return {Object}                    Containing all the information of the request but the payload.c
 */
function createRequestObject(url, typeInformation, token, updateFlag) {
...
function sendQueryValue(entityName, attributes, typeInformation, token, callback) {
    var url= '/v2/entities/'+entityName;

    if(attributes){
        var ngsiv2AttributesParameter='';

        for(var i=0;i<attributes.length;i++){
            ngsiv2AttributesParameter= ngsiv2AttributesParameter+attributes[i];
            if(i<attributes.length-1){
                ngsiv2AttributesParameter= ngsiv2AttributesParameter+','
            }
        }

        if(attributes.length>0){
            url=url+'?attrs='+ngsiv2AttributesParameter;
        }
    }

    var options = createRequestObject(url, typeInformation, token,false);
