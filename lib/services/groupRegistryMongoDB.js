...
function createGroup(group, callback) {
    ...
    groupObj.save(saveGroupHandler(callback));
}
...
function find(service, subservice, callback) {
    ...
    logger.debug(context, 'Looking for entity with service name [%s] and subservice [%s].', service, subservice);

    query = Group.model.findOne({
        service: service,
    ...
    query.exec(function handleGet(error, data) {
        if (error) {
            ...
            logger.debug(context, 'Device group for service [%s] and subservice [%s] not found.', service, subservice);
