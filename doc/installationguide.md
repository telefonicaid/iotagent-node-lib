...
* **authentication**: authentication data, for use in retrieving tokens for devices with a trust token (required in scenarios with security enabled in the Context Broker side). 
  Currently, two authentication provider are supported: `keystone` and `oauth2`. 
  ...
  combination of `fiware-service` and `service-path`). The authentication process use the trust delegation workflow to check if the trust provided is valid, in which
  ...
  case of security issues connected to a token). The code was tested using Keycloak and Auth0 (it may require customisation
