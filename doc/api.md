...
| `explicitAttrs`                | `explicitAttrs`                | optional boolean value, to support selective ignore of measures so that IOTA doesn’t progress. If not specified default is `false`. Additionally `explicitAttrs` can be used to define which meassures defined in JSON/JEXL array will be propagated to NGSI interface. [See all cases](advanced-topics.md#explicitly-defined-attributes-explicitattrs)  |
...
| `expressionLanguage`  | `expresionLanguage`  | optional boolean value, to set expression language used to compute expressions, possible values are: legacy or jexl. When not set or wrongly set, legacy is used as default value.                                                                                        |
| `explicitAttrs`       | `explicitAttrs`      | Optional boolean value to support selective ignore of measures for device so that IOTA doesn’t progress. If not specified default is `false`. Additionally `explicitAttrs` can be used to define which meassures defined in JSON/JEXL array will be propagated to NGSI interface. [See all cases](advanced-topics.md#explicitly-defined-attributes-explicitattrs)                                                                            | `true/false`                                  |
